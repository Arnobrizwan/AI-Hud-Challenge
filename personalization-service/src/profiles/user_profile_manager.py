...
from ..models.schemas import UserProfile, UserInteraction, InteractionType
...
class UserProfileManager:
    ...
    async def update_profile_from_interaction(self, user_id: str, 
        ...
        if interaction.interaction_type in [InteractionType.CLICK, InteractionType.SHARE, InteractionType.SAVE]:
    ...
    async def update_topic_preferences(self, current_preferences: Dict[str, float],
                                     topics: List[str], 
                                     interaction_type: InteractionType) -> Dict[str, float]:
        ...
        if total_weight > 0:
            current_preferences = {
                ...
                for topic, weight in current_preferences.items()
            }
        
        return current_preferences
    
    async def update_source_preferences(self, current_preferences: Dict[str, float],
                                      source: str, 
                                      interaction_type: InteractionType) -> Dict[str, float]:
        ...
        if total_weight > 0:
            current_preferences = {
                ...
                for source, weight in current_preferences.items()
            }
        
        return current_preferences
    
    async def update_reading_patterns(self, current_patterns: Dict[str, Any],
    ...
    async def get_profile_analytics(self) -> Dict[str, Any]:
        ...
        # Get total users
        total_users = await self.postgres.fetch_one(
