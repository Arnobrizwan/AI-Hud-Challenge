...
class ContentBasedFilter:
    ...
    async def initialize(self) -> None:
        ...
        if content_items:
            ...
            await asyncio.gather(
                self.tfidf_filter.fit(content_items),
                self.embedding_filter.fit(content_items)
            )
    ...
    async def get_recommendations(self, user_profile: UserProfile, 
                                n_recommendations: int = 10) -> List[Recommendation]:
        ...
        await self.redis.setex(
            cache_key, 
            self.cache_ttl, 
            [rec.dict() for rec in top_recommendations]
        )
        
        return top_recommendations
    
    async def _get_recommendation_candidates(self, user_profile: UserProfile, 
    ...
    async def _extract_recommendation_features(self, content_item: ContentItem, 
