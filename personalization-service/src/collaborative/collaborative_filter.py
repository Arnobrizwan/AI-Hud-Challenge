...
from ..models.schemas import UserInteraction, SimilarUser, Recommendation, ContentItem, UserProfile
...
class CollaborativeFilter:
    ...
    async def get_similar_users(self, user_id: str, n_users: int = 10) -> List[SimilarUser]:
        ...
        await self.redis.setex(
            cache_key, 
            self.cache_ttl, 
            [user.dict() for user in similar_users]
        )
    ...
    async def get_recommendations(self, user_id: str, n_recommendations: int = 10) -> List[Recommendation]:
        ...
        await self.redis.setex(
            cache_key, 
            self.cache_ttl, 
            [rec.dict() for rec in rec_objects]
        )
