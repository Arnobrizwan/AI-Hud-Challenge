...
from ..models.schemas import Recommendation, UserProfile, DiversityParams, ContentItem
...
class DiversityOptimizer:
    ...
    async def optimize(self, recommendations: List[Recommendation], 
                      user_profile: UserProfile,
                      diversity_params: Optional[DiversityParams] = None) -> List[Recommendation]:
        ...
        diversified = await self._apply_diversity_constraints(
            recommendations, diversity_params
        )
    ...
    async def _apply_diversity_constraints(self, recommendations: List[Recommendation],
                                         diversity_params: DiversityParams) -> List[Recommendation]:
    ...
    async def _check_topic_diversity(self, recommendation: Recommendation,
    ...
    async def _check_source_diversity(self, recommendation: Recommendation,
    ...
    async def _apply_serendipity_optimization(self, recommendations: List[Recommendation],
                                            user_profile: UserProfile,
                                            diversity_params: DiversityParams) -> List[Recommendation]:
    ...
    async def _compute_serendipity_boost(self, recommendation: Recommendation,
                                       user_profile: UserProfile,
                                       serendipity_weight: float) -> float:
        """Compute serendipity boost for a recommendation."""
        # Topic serendipity
        topic_serendipity = await self._compute_topic_serendipity(
            recommendation, user_profile
        )
        
        # Source serendipity
        source_serendipity = await self._compute_source_serendipity(
            recommendation, user_profile
        )
        
        # Content novelty
        content_novelty = await self._compute_content_novelty(
            recommendation, user_profile
        )
        
        # Combine serendipity factors
        serendipity_score = (
            topic_serendipity * 0.4 +
            source_serendipity * 0.3 +
            content_novelty * 0.3
        )
        
        return serendipity_score * serendipity_weight
    
    async def _compute_topic_serendipity(self, recommendation: Recommendation,
    ...
    async def _compute_source_serendipity(self, recommendation: Recommendation,
                                        user_profile: UserProfile) -> float:
    ...
    async def _compute_content_novelty(self, recommendation: Recommendation,
                                     user_profile: UserProfile) -> float:
        ...
        if 'embedding_dim' in recommendation.features:
            ...
            embedding_values = [
                v for k, v in recommendation.features.items() 
                if k.startswith('embedding_dim_')
            ]
            if embedding_values:
    ...
    async def _compute_serendipity_score(self, recommendation: Recommendation,
                                       user_profile: UserProfile) -> float:
    ...
    async def _rerank_with_diversity(self, recommendations: List[Recommendation],
                                   user_profile: UserProfile,
                                   diversity_params: DiversityParams) -> List[Recommendation]:
    ...
    async def _compute_diversity_scores(self, recommendations: List[Recommendation]) -> List[float]:
        ...
        for i, rec in enumerate(recommendations):
            ...
            for j, other_rec in enumerate(recommendations):
                if i != j:
                    # Topic diversity
                    topic_diversity = await self._compute_topic_diversity_score(
                        rec, other_rec
                    )
                    
                    # Source diversity
                    source_diversity = await self._compute_source_diversity_score(
                        rec, other_rec
                    )
                    
                    # Content diversity (using features)
                    content_diversity = await self._compute_content_diversity_score(
                        rec, other_rec
                    )
    ...
    async def _compute_topic_diversity_score(self, rec1: Recommendation,
                                           rec2: Recommendation) -> float:
    ...
    async def _compute_source_diversity_score(self, rec1: Recommendation,
                                            rec2: Recommendation) -> float:
    ...
    async def _compute_content_diversity_score(self, rec1: Recommendation,
                                             rec2: Recommendation) -> float:
