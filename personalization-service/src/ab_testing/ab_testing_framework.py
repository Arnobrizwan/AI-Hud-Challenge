...
class ABTestingFramework:
    ...
    async def _assign_user_to_variant(self, user_id: str, experiment_data: Dict[str, Any]) -> str:
        ...
        for variant, probability in traffic_allocation.items():
            ...
            if user_hash <= cumulative_prob:
                # Record assignment
                await self._record_user_assignment(user_id, experiment_data['experiment_id'], variant)
                return variant
    ...
    async def _calculate_variant_metrics(self, users: List[Dict[str, Any]], 
    ...
    async def _calculate_statistical_significance(self, variant_metrics: Dict[str, Any]) -> Dict[str, Any]:
