...
class EscalationEngine:
    ...
    async def setup_escalation(self, alert: Alert, routing_decision: Dict[str, Any]):
        ...
        self.active_escalations[escalation_id] = {
            ...
            'next_escalation': datetime.utcnow() + timedelta(minutes=policy['levels'][alert.escalation_level]['delay_minutes']),
            'created_at': datetime.utcnow()
        }
...
class AlertingSystem:
    ...
    async def calculate_alert_severity(self, alert: Alert, correlated_alerts: List[Alert]) -> AlertSeverity:
        ...
        recent_alerts = [a for a in correlated_alerts 
