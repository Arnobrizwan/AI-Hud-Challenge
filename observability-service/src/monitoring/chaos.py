...
class ChaosEngine:
    ...
    async def _execute_network_latency_experiment(self, experiment: ChaosExperiment, 
                                                execution: ExperimentExecution):
    ...
    async def _execute_cpu_stress_experiment(self, experiment: ChaosExperiment, 
                                           execution: ExperimentExecution):
    ...
    async def _execute_memory_stress_experiment(self, experiment: ChaosExperiment, 
                                              execution: ExperimentExecution):
        ...
        logger.info(f"Simulating memory stress of {memory_percentage}% for {duration_minutes} minutes")
    ...
    async def _execute_service_failure_experiment(self, experiment: ChaosExperiment, 
                                                execution: ExperimentExecution):
        ...
        logger.info(f"Simulating failure of services: {target_services} for {failure_duration_minutes} minutes")
    ...
    async def _execute_database_failure_experiment(self, experiment: ChaosExperiment, 
                                                 execution: ExperimentExecution):
        ...
        logger.info(f"Simulating {database_type} database failure for {failure_duration_minutes} minutes")
    ...
    async def _execute_random_kill_experiment(self, experiment: ChaosExperiment, 
                                            execution: ExperimentExecution):
    ...
    async def _generate_reliability_report(self, experiment: ChaosExperiment, 
                                         execution: ExperimentExecution) -> ReliabilityReport:
        """Generate reliability report for experiment"""
        
        # Calculate reliability score based on metrics
        reliability_score = self._calculate_reliability_score(
            execution.metrics_before,
            execution.metrics_during,
    ...
    def _calculate_reliability_score(self, metrics_before: Dict[str, Any], 
        ...
        cpu_degradation = abs(metrics_during.get('cpu_usage', 0) - metrics_before.get('cpu_usage', 0))
        memory_degradation = abs(metrics_during.get('memory_usage', 0) - metrics_before.get('memory_usage', 0))
        
        # Calculate recovery
        cpu_recovery = abs(metrics_after.get('cpu_usage', 0) - metrics_before.get('cpu_usage', 0))
        memory_recovery = abs(metrics_after.get('memory_usage', 0) - metrics_before.get('memory_usage', 0))
    ...
    def _generate_reliability_recommendations(self, experiment: ChaosExperiment, 
                                            execution: ExperimentExecution, 
    ...
    async def get_reliability_summary(self) -> Dict[str, Any]:
        ...
        recent_scores = [
            e.results.get('overall_reliability_score', 0) for e in recent_executions
        ...
        return {
            ...
            'last_experiment': completed_executions[-1].started_at.isoformat() if completed_executions else None
        }
