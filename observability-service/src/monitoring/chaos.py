...
class ChaosEngine:
    ...
    async def load_experiment_definitions(self, experiments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _generate_reliability_report(
        ...
        recommendations = self._generate_reliability_recommendations(experiment, execution, reliability_score)
    ...
    async def get_experiment_results(self, experiment_id: str) -> List[ExperimentExecution]:
        ...
        return [execution for execution in self.executions.values() if execution.experiment_id == experiment_id]

    async def get_reliability_summary(self) -> Dict[str, Any]:
        """Get overall reliability summary"""
        completed_executions = [e for e in self.executions.values() if e.status == ExperimentStatus.COMPLETED]
