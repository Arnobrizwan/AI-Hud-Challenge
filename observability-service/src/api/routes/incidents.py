...
from monitoring.incidents import IncidentManager, Incident, IncidentStatus, IncidentSeverity, IncidentType
...
async def create_emergency_incident(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create emergency incident: {str(e)}")
...
async def get_incidents(
    ...
    try:
        ...
        if incident_type:
            filtered_incidents = [i for i in filtered_incidents if i.incident_type.value == incident_type]
...
async def get_incident(incident_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get incident {incident_id}: {str(e)}")
...
async def update_incident(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update incident {incident_id}: {str(e)}")
...
async def get_incident_timeline(incident_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get timeline for incident {incident_id}: {str(e)}")
...
async def create_post_mortem(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create post-mortem for incident {incident_id}: {str(e)}")
...
async def get_post_mortem(incident_id: str):
    ...
    try:
        ...
        post_mortem = next((pm for pm in post_mortems.values() if pm.incident_id == incident_id), None)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get post-mortem for incident {incident_id}: {str(e)}")
...
async def get_incident_statistics(
    ...
    try:
        ...
        metrics = await observability_engine.incident_manager.get_incident_metrics(time_window_delta)
