...
import uvicorn
from api.routes import alerts, chaos, cost, health, incidents, metrics, runbooks, slo
...
async def trigger_emergency_response(
    incident_type: str, severity: str, description: str, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        background_tasks.add_task(incident_manager.trigger_emergency_response, incident.id)
...
if __name__ == "__main__":
    settings = get_settings()
    uvicorn.run(
        "main:app", host="0.0.0.0", port=settings.port, reload=settings.debug, log_level="info"
    )
