...
class SummarizationRequest(BaseModel):
    ...
    target_lengths: List[int] = Field(
        default=[50, 120, 300],
        description="Target summary lengths in words"
    )
    methods: List[SummarizationMethod] = Field(
        default=[SummarizationMethod.HYBRID],
        description="Summarization methods to use"
    )
    ...
    enable_factual_consistency: bool = Field(
        default=True,
        description="Enable factual consistency checking"
    )
    custom_prompts: Optional[Dict[str, str]] = Field(
        None,
        description="Custom prompts for different methods"
    )
...
class SummaryResult(BaseModel):
    ...
    variants: List[Union[Summary, Headline]] = Field(
        ...
        description="All generated variants"
    )
    ...
    source_attribution: List[str] = Field(
        default_factory=list,
        description="Source attribution and citations"
    )
...
class A/BTestVariant(BaseModel):
