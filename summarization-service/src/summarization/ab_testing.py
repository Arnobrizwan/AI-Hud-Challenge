...
from .models import A/BTestVariant, A/BTestResult, SummarizationRequest, SummaryResult
...
class ABTestManager:
    ...
    async def _save_test_data(self):
        ...
        logger.info(f"Saving {len(self.active_tests)} tests and {sum(len(results) for results in self.test_results.values())} results")
