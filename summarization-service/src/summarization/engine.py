...
from .models import (
    SummarizationRequest, SummaryResult, Summary, ProcessedContent,
    SummarizationMethod, QualityMetrics, BiasAnalysis, ConsistencyScores,
    ProcessingStats, Language, ContentType
)
...
class ContentSummarizationEngine:
    ...
    async def generate_summary(self, request: SummarizationRequest) -> SummaryResult:
        ...
        try:
            ...
            headlines = await self.generate_headlines(
                processed_content, request.headline_styles
            )
            processing_stats.headline_generation_time = time.time() - headline_start
            
            # Step 6: Quality validation
            quality_start = time.time()
            quality_metrics = await self.validate_quality(
    ...
    async def validate_quality(
        ...
        try:
            ...
            return await self.quality_scorer.score_quality(
                content.text, best_summary.text
            )
        except Exception as e:
    ...
    async def analyze_bias(self, content_variants: List[Any]) -> BiasAnalysis:
        ...
        try:
            ...
            all_text = " ".join([getattr(variant, 'text', str(variant)) for variant in content_variants])
    ...
    async def check_consistency(
        ...
        try:
            ...
            summary_text = next(
                (getattr(v, 'text', str(v)) for v in variants if hasattr(v, 'text')), 
                content.text
            )
    ...
    def select_best_summary(self, summaries: List[Summary], quality_metrics: QualityMetrics) -> Summary:
        ...
        try:
            ...
            return SummaryResult(
                ...
                quality_metrics=QualityMetrics(
                    ...
                    bertscore_f1=0.0, factual_consistency=0.0,
                    readability=0.0, coverage=0.0, abstractiveness=0.0,
                    overall_score=0.5
                ),
                consistency_scores=ConsistencyScores(
                    entity_consistency=0.0, numerical_consistency=0.0,
                    temporal_consistency=0.0, entailment_score=0.0,
