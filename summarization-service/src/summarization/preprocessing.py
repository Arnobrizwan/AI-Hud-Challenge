...
from .models import ProcessedContent, Language, ContentType
    ...
    async def _detect_content_type(self, text: str) -> ContentType:
        ...
        try:
            ...
            news_indicators = [
                ...
                'press release', 'news', 'update'
            ]
            
            # Blog post indicators
            blog_indicators = [
                'i think', 'in my opinion', 'personally', 'i believe',
                'from my experience', 'i\'ve learned', 'tips', 'guide',
                'tutorial', 'how to', 'why', 'what'
            ]
            
            # Academic paper indicators
            academic_indicators = [
                'abstract', 'introduction', 'methodology', 'results',
                'conclusion', 'references', 'study', 'research',
                'analysis', 'findings', 'hypothesis', 'data'
            ]
            
            # Social media indicators
            social_indicators = [
                'hashtag', '@', 'retweet', 'like', 'share',
                'follow', 'unfollow', 'dm', 'status', 'post'
            ]
            
            # Product description indicators
            product_indicators = [
                'buy', 'purchase', 'price', 'sale', 'discount',
                'features', 'specifications', 'warranty', 'shipping',
                'product', 'item', 'order'
            ]
    ...
    async def _extract_metadata(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if metadata['sentence_count'] > 0:
                metadata['avg_sentence_length'] = metadata['word_count'] / metadata['sentence_count']
            else:
