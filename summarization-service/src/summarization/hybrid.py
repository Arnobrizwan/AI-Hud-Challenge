...
class HybridSummarizer:
    ...
    async def _weighted_hybrid(self, content: ProcessedContent, target_length: int) -> str:
        ...
        try:
            ...
            combined_summary = await self._combine_summaries(
                extractive_summary, abstractive_summary, 0.5, 0.5, target_length
            )
    ...
    async def generate_multiple_variants(
        self, content: ProcessedContent, target_lengths: List[int]
    ) -> List[Dict[str, Any]]:
            ...
            for strategy in strategies:
                try:
                    summary = await self.summarize(
                        content=content, target_length=target_length, strategy=strategy
                    )
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "extractive_available": (
                self.extractive_summarizer._initialized
                if hasattr(self.extractive_summarizer, "_initialized")
                else False
            ),
            "abstractive_available": (
                self.abstractive_summarizer._initialized
                if hasattr(self.abstractive_summarizer, "_initialized")
                else False
            ),
            "strategies": ["adaptive", "weighted", "sequential"],
        }
