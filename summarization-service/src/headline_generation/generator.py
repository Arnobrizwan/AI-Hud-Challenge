...
from summarization.models import ProcessedContent, HeadlineStyle, Language
...
class HeadlineGenerator:
        ...
        try:
            ...
            for style in styles:
                try:
                    ...
                    scored_candidates = await self._score_headlines(
                        candidates, content, style_enum
                    )
    ...
    async def _score_headlines(
        ...
        try:
            ...
            return [{'text': candidate, 'style': style.value, 'score': 0.5, 'metrics': {}} 
                   for candidate in candidates]
    ...
    async def _check_factual_accuracy(self, headline: str, content: str) -> float:
        ...
        try:
            ...
            headline_words = set(word.lower() for word in word_tokenize(headline) 
                               if word.isalpha() and len(word) > 2)
            content_words = set(word.lower() for word in word_tokenize(content) 
                              if word.isalpha() and len(word) > 2)
    ...
    async def _predict_engagement(self, headline: str, style: HeadlineStyle) -> float:
        ...
        try:
            ...
            engagement_indicators = [
                'amazing', 'shocking', 'incredible', 'breaking', 'urgent',
                'exclusive', 'revealed', 'uncovered', 'exposed', 'secret',
                'must-see', 'can\'t miss', 'essential', 'critical', 'important'
            ]
            ...
            emotional_words = [
                ...
                'excitement', 'worry', 'hope', 'disappointment', 'relief'
            ]
    ...
    def _check_style_adherence(self, headline: str, style: HeadlineStyle) -> float:
        ...
        try:
            ...
            keyword_matches = sum(1 for keyword in style_config['keywords'] 
            ...
            if style == HeadlineStyle.QUESTION and headline.endswith('?'):
                pattern_score = 1.0
            elif style == HeadlineStyle.URGENT and any(word in headline_lower 
                                                     for word in ['breaking', 'urgent', 'alert']):
                pattern_score = 1.0
            elif style == HeadlineStyle.ENGAGING and any(word in headline_lower 
