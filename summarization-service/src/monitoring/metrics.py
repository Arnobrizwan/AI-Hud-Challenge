...
from summarization.models import SummaryResult, QualityMetrics, ProcessingStats
...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        # A/B test metrics
        self.ab_test_metrics = defaultdict(lambda: {
            "participants": 0,
            "conversions": 0,
            "avg_quality": 0.0,
            "avg_processing_time": 0.0
        ...
        try:
            ...
            self.method_metrics[method]["avg_quality"] = (
                (self.method_metrics[method]["avg_quality"] * (self.method_metrics[method]["count"] - 1) + 
                 result.quality_metrics.overall_score) / self.method_metrics[method]["count"]
            )
            ...
            self.language_metrics[language]["avg_processing_time"] = (
                (self.language_metrics[language]["avg_processing_time"] * (self.language_metrics[language]["count"] - 1) + 
                 result.processing_stats.total_time) / self.language_metrics[language]["count"]
            )
            self.language_metrics[language]["avg_quality"] = (
                (self.language_metrics[language]["avg_quality"] * (self.language_metrics[language]["count"] - 1) + 
                 result.quality_metrics.overall_score) / self.language_metrics[language]["count"]
            )
    ...
    def _update_quality_metrics(self, quality: QualityMetrics):
        ...
        try:
            ...
            self.quality_metrics.average_rouge1 = (
                (self.quality_metrics.average_rouge1 * (n - 1) + quality.rouge1_f1) / n
            )
            self.quality_metrics.average_rouge2 = (
                (self.quality_metrics.average_rouge2 * (n - 1) + quality.rouge2_f1) / n
            )
            self.quality_metrics.average_rougeL = (
                (self.quality_metrics.average_rougeL * (n - 1) + quality.rougeL_f1) / n
            )
            self.quality_metrics.average_bertscore = (
                (self.quality_metrics.average_bertscore * (n - 1) + quality.bertscore_f1) / n
            )
            self.quality_metrics.average_consistency = (
                (self.quality_metrics.average_consistency * (n - 1) + quality.factual_consistency) / n
            )
            self.quality_metrics.average_readability = (
                (self.quality_metrics.average_readability * (n - 1) + quality.readability) / n
            )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_hit_rate = (
                ...
                if (self.service_metrics.cache_hits + self.service_metrics.cache_misses) > 0 else 0
            )
