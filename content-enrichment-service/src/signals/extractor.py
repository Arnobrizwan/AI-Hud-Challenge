...
class SignalExtractor:
    ...
    def _load_bias_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "left_bias": [
                "progressive", "liberal", "democrat", "left-wing", "socialist",
                "equality", "social justice", "climate change", "renewable energy"
            ],
            "right_bias": [
                "conservative", "republican", "right-wing", "traditional",
                "free market", "small government", "patriotism", "family values"
            ],
            "emotional_language": [
                "shocking", "outrageous", "incredible", "amazing", "terrible",
                "horrible", "fantastic", "devastating", "stunning", "appalling"
            ],
    ...
    def _load_political_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "left": [
                "progressive", "liberal", "democrat", "socialist", "green party",
                "climate action", "social justice", "universal healthcare"
            ],
            "right": [
                "conservative", "republican", "libertarian", "tea party",
                "free market", "small government", "traditional values"
            ],
            "center": [
                "moderate", "centrist", "bipartisan", "compromise", "balanced"
            ]
        }
    ...
    def _load_expertise_indicators(self) -> List[str]:
        ...
        return [
            "research", "study", "data", "statistics", "analysis", "report",
            "expert", "professor", "doctor", "scientist", "researcher",
            "peer-reviewed", "journal", "university", "institute", "laboratory",
            "clinical trial", "experiment", "findings", "conclusions"
        ]
    ...
    async def extract_signals(
        ...
        try:
            ...
            logger.info("Content signals extracted",
            ...
            logger.error("Signal extraction failed",
                        content_id=content.id,
                        error=str(e))
    ...
    async def _analyze_bias(self, text: str) -> float:
        ...
        try:
            ...
            # Count bias keywords
            left_count = sum(1 for keyword in self.bias_keywords["left_bias"] 
                           if keyword in text_lower)
            right_count = sum(1 for keyword in self.bias_keywords["right_bias"] 
                            if keyword in text_lower)
            
            # Count emotional language
            emotional_count = sum(1 for keyword in self.bias_keywords["emotional_language"] 
                                if keyword in text_lower)
            
            # Count subjective indicators
            subjective_count = sum(1 for keyword in self.bias_keywords["subjective_indicators"] 
                                 if keyword in text_lower)
    ...
    async def _detect_political_leaning(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            left_score = sum(1 for keyword in self.political_keywords["left"] 
                           if keyword in text_lower)
            right_score = sum(1 for keyword in self.political_keywords["right"] 
                            if keyword in text_lower)
            center_score = sum(1 for keyword in self.political_keywords["center"] 
                             if keyword in text_lower)
    ...
    async def _calculate_virality_potential(
        self, 
        text: str, 
        content: ExtractedContent
    ) -> float:
        ...
        try:
            ...
            # Emotional words
            emotional_words = [
                "viral", "trending", "breaking", "shocking", "amazing", "incredible",
                "unbelievable", "stunning", "devastating", "outrageous", "controversial"
            ]
            ...
            controversial_topics = [
                "politics", "scandal", "controversy", "outrage", "backlash",
                "criticism", "debate", "dispute", "conflict", "crisis"
            ]
            
            controversial_count = sum(1 for topic in controversial_topics 
                                    if topic in text_lower)
            virality_score += min(0.2, controversial_count * 0.05)
            
            # Social media indicators
            social_indicators = ["share", "retweet", "like", "comment", "follow"]
            social_count = sum(1 for indicator in social_indicators 
                             if indicator in text_lower)
            virality_score += min(0.1, social_count * 0.02)
            
            # Time sensitivity
            time_sensitive = [
                "breaking", "urgent", "immediate", "now", "today", "latest",
                "just in", "recently", "newly", "fresh"
            ]
    ...
    async def _calculate_authority_score(
        ...
        try:
            ...
            if content.author:
                ...
                professional_titles = [
                    "dr", "professor", "prof", "doctor", "phd", "md", "rn",
                    "expert", "specialist", "analyst", "researcher"
                ]
                
                author_lower = content.author.lower()
                title_count = sum(1 for title in professional_titles 
            ...
            if content.source:
                ...
                credible_sources = [
                    "reuters", "ap", "bbc", "cnn", "nytimes", "washington post",
                    "wall street journal", "bloomberg", "forbes", "techcrunch"
                ]
            ...
            if content.url:
                try:
                    ...
                    trusted_domains = [
                        ".edu", ".gov", ".org", "reuters.com", "ap.org",
            ...
            expertise_count = sum(1 for indicator in self.expertise_indicators 
    ...
    async def compute_trustworthiness(
        ...
        try:
            ...
            logger.info("Trustworthiness score computed",
                       content_id=content.id,
                       overall_score=overall_score,
                       source_reliability=source_reliability)
            ...
            logger.error("Trustworthiness computation failed",
                        content_id=content.id,
                        error=str(e))
        ...
        try:
            ...
            if content.source:
                ...
                reliable_sources = [
                    ...
                    "bloomberg", "forbes", "techcrunch", "wired"
                ]
    ...
    async def _assess_author_credibility(self, content: ExtractedContent) -> float:
        ...
        try:
            ...
            professional_titles = [
                "dr", "professor", "prof", "doctor", "phd", "md", "rn",
                "expert", "specialist", "analyst", "researcher", "scientist"
            ]
            ...
            institutions = [
                ...
                "research center", "think tank", "ngo", "government"
            ]
            
            institution_count = sum(1 for inst in institutions 
    ...
    async def _identify_bias_indicators(
        ...
        try:
            ...
            emotional_count = sum(1 for word in self.bias_keywords["emotional_language"] 
                                if word in text)
            if emotional_count > 3:
                indicators.append("excessive_emotional_language")
            
            # Subjective language
            subjective_count = sum(1 for word in self.bias_keywords["subjective_indicators"] 
                                 if word in text)
