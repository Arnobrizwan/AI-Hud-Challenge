        ...
        try:
            ...
            if identifier in self.local_cache:
                self.local_cache[identifier] = [
                    timestamp for timestamp in self.local_cache[identifier] if timestamp > window_start
                ]
            else:
                self.local_cache[identifier] = []
    ...
    async def get_rate_limit_info(self, identifier: str) -> Dict[str, int]:
        ...
        try:
            if self.redis_client:
                ...
                # Get current count
                current_count = await self.redis_client.zcount(f"rate_limit:{identifier}", window_start, current_time)
