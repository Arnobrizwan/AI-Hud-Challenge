...
class EntityExtractor:
    ...
    def _load_custom_patterns(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {
            "email": [
                {"pattern": r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', "label": "EMAIL"}
            ],
            "phone": [
                {"pattern": r'(\+?1[-.\s]?)?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}', "label": "PHONE"}
            ],
            "url": [
                {"pattern": r'https?://[^\s<>"{}|\\^`\[\]]+', "label": "URL"}
            ],
            "hashtag": [
                {"pattern": r'#\w+', "label": "HASHTAG"}
            ],
            "mention": [
                {"pattern": r'@\w+', "label": "MENTION"}
            ],
            "cryptocurrency": [
                {"pattern": r'\b(BTC|ETH|LTC|XRP|ADA|DOT|LINK|UNI|AAVE|COMP)\b', "label": "CRYPTO"}
            ],
            "stock_ticker": [
                {"pattern": r'\b[A-Z]{1,5}\b', "label": "STOCK"}  # Basic pattern, would need validation
            ]
        }
    ...
    def _calculate_confidence(self, ent: Any, entity_type: EntityType) -> float:
        ...
        if hasattr(ent, 'sent') and ent.sent:
            ...
            if entity_type == EntityType.PERSON and any(title in sent_text for title in ['mr', 'ms', 'dr', 'prof']):
                context_factor = 1.2
            elif entity_type == EntityType.ORGANIZATION and any(word in sent_text for word in ['inc', 'corp', 'ltd', 'llc']):
                context_factor = 1.1
