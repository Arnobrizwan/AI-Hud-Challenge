...
from ..models.content import ExtractedContent, ContentType, SentimentLabel, EmotionLabel
...
class TestSentimentAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_analyze_sentiment_positive(self, analyzer, positive_content):
    ...
    @pytest.mark.asyncio
    async def test_analyze_sentiment_negative(self, analyzer, negative_content):
    ...
    @pytest.mark.asyncio
    async def test_analyze_sentiment_neutral(self, analyzer, neutral_content):
        """Test sentiment analysis for neutral content."""
        sentiment = await analyzer.analyze_sentiment(neutral_content)
        
        assert sentiment.sentiment in [SentimentLabel.NEUTRAL, SentimentLabel.POSITIVE, SentimentLabel.NEGATIVE]
        assert sentiment.confidence > 0.0
        assert sentiment.subjectivity < 0.5  # Should be more objective
        assert -0.2 < sentiment.polarity < 0.2  # Should be close to neutral
    
    @pytest.mark.asyncio
    async def test_analyze_sentiment_with_emotions(self, analyzer, positive_content):
    ...
    @pytest.mark.asyncio
    async def test_analyze_sentiment_batch(self, analyzer, positive_content, negative_content, neutral_content):
    ...
    @pytest.mark.asyncio
    async def test_analyze_sentiment_fallback(self, analyzer, positive_content):
    ...
    def test_get_sentiment_statistics(self, analyzer):
        ...
        sentiments = [
            Mock(sentiment=SentimentLabel.POSITIVE, confidence=0.8, subjectivity=0.6, polarity=0.7, emotions={EmotionLabel.JOY: 0.8}),
            Mock(sentiment=SentimentLabel.NEGATIVE, confidence=0.9, subjectivity=0.8, polarity=-0.6, emotions={EmotionLabel.ANGER: 0.7}),
            Mock(sentiment=SentimentLabel.NEUTRAL, confidence=0.7, subjectivity=0.3, polarity=0.1, emotions={EmotionLabel.NEUTRAL: 1.0}),
        ]
    ...
    @pytest.mark.asyncio
    async def test_detect_sentiment_shift(self, analyzer):
        ...
        assert shift["has_significant_change"] == True
    
    @pytest.mark.asyncio
    async def test_analyze_sentiment_multilingual(self, analyzer):
