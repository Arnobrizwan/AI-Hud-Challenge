...
class BayesianTestingFramework:
    ...
    async def create_bayesian_test(self, 
    ...
    async def _normal_normal_test(self, test: Dict[str, Any]):
        ...
        if len(control_data) > 0:
            ...
            control_posterior_mean = (prior_precision * prior_mean + len(control_data) * data_precision * control_mean) / control_precision
        ...
        if len(treatment_data) > 0:
            ...
            treatment_posterior_mean = (prior_precision * prior_mean + len(treatment_data) * data_precision * treatment_mean) / treatment_precision
    ...
    async def _check_stopping_conditions(self, 
                                       test: Dict[str, Any], 
                                       prob_better: float, 
                                       expected_loss: float) -> Dict[str, Any]:
