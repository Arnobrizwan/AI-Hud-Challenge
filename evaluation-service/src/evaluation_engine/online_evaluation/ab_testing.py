...
from ..models import (
    Experiment, ExperimentConfig, ExperimentData, ExperimentAnalysis,
    VariantAnalysis, FrequentistAnalysis, BayesianAnalysis, SequentialAnalysis,
    ExperimentStatus, StatisticalTestType
)
from ..statistical_testing import StatisticalTester, PowerAnalyzer
...
class ABTestingFramework:
    ...
    async def analyze_experiment(self, 
                               experiment_id: str,
                               analysis_type: str = 'frequentist') -> ExperimentAnalysis:
    ...
    async def _frequentist_analysis(self, 
                                  experiment: Experiment,
                                  experiment_data: ExperimentData) -> FrequentistAnalysis:
            ...
            for metric in experiment.secondary_metrics:
                ...
                if len(control_metric_values) > 0 and len(variant_metric_values) > 0:
                    secondary_test_result = await self.statistical_tester.two_sample_test(
                        control_metric_values, variant_metric_values,
                        test_type=StatisticalTestType.WELCH_T_TEST
                    )
        ...
        overall_significance = any(
            result.primary_metric_result.get('is_significant', False)
    ...
    async def _bayesian_analysis(self, 
                               experiment: Experiment,
                               experiment_data: ExperimentData) -> BayesianAnalysis:
    ...
    async def _sequential_analysis(self, 
                                 experiment: Experiment,
                                 experiment_data: ExperimentData) -> SequentialAnalysis:
    ...
    async def _analyze_guardrail_metrics(self, 
                                       experiment: Experiment,
                ...
                if len(control_values) > 0 and len(variant_values) > 0:
                    ...
                    metric_analysis[variant_name] = {
                        'is_significantly_worse': (
                            test_result.get('is_significant', False) and 
                            np.mean(variant_values) < np.mean(control_values)
                        ),
    ...
    async def _generate_experiment_recommendations(self, 
                                                 analysis, 
                                                 significance_results: Dict[str, Any],
                                                 guardrail_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if significance_results.get('overall_significant', False):
            if isinstance(analysis, FrequentistAnalysis):
                ...
                winning_variants = [
                    variant for variant, result in analysis.variant_results.items()
    ...
    def _calculate_estimated_end_date(self, sample_size: int, config: ExperimentConfig) -> datetime:
        ...
        # Estimate based on daily traffic (this would come from actual traffic data)
