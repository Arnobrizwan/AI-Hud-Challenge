...
class CrossValidator:
    ...
    async def cross_validate_model(self, 
        ...
        if return_train_score:
            results['overfitting_indicator'] = (
                results['train_mean'] - results['test_mean']
            ) / results['test_std'] if results['test_std'] > 0 else 0
        
        return results
    
    async def nested_cross_validation(self, 
                                    model, 
                                    X: np.ndarray, 
                                    y: np.ndarray,
    ...
    async def learning_curve_analysis(self, 
                                    model, 
                                    X: np.ndarray, 
                                    y: np.ndarray,
    ...
    async def validation_curve_analysis(self, 
                                      model, 
                                      X: np.ndarray, 
                                      y: np.ndarray,
                                      param_name: str,
    ...
    async def bootstrap_validation(self, 
                                 model, 
                                 X: np.ndarray, 
                                 y: np.ndarray,
                                 n_bootstrap: int = 100,
