...
class CrossValidator:
    ...
    async def cross_validate_model(
        ...
        if return_train_score:
            results["overfitting_indicator"] = (
                (results["train_mean"] - results["test_mean"]) / results["test_std"] if results["test_std"] > 0 else 0
            )
    ...
    async def nested_cross_validation(
        ...
        for train_idx, test_idx in outer_cv_folds.split(X, y):
            ...
            grid_search = GridSearchCV(model, param_grid, cv=inner_cv_folds, scoring=scoring, n_jobs=-1)
