...
class FeatureEvaluator:
    ...
    async def evaluate_feature_importance(
        self, model, dataset: Optional[Dict[str, Any]] = None
    ) -> Dict[str, float]:
        ...
        if hasattr(model, "feature_importances_"):
            # Tree-based models
            importance_scores["tree_importance"] = dict(
    ...
    async def _mutual_information_importance(
    ...
    async def _aggregate_importance_scores(
        self, importance_scores: Dict[str, Dict[int, float]]
    ) -> Dict[int, float]:
