...
class PowerAnalyzer:
    ...
    async def calculate_sample_size(
        ...
        sample_size = await self.power_methods[test_type](effect_size, alpha, power, baseline_conversion_rate)

        return max(sample_size, 100)  # Minimum sample size

    async def _t_test_power(self, effect_size: float, alpha: float, power: float, baseline_rate: float) -> int:
    ...
    async def _z_test_power(self, effect_size: float, alpha: float, power: float, baseline_rate: float) -> int:
    ...
    async def _chi_square_power(self, effect_size: float, alpha: float, power: float, baseline_rate: float) -> int:
    ...
    async def _anova_power(self, effect_size: float, alpha: float, power: float, baseline_rate: float) -> int:
    ...
    def _generate_power_recommendations(self, power: float, sample_size: int, required_n: int) -> List[str]:
        ...
        if sample_size < 1000:
            recommendations.append("Sample size is relatively small - consider increasing for more reliable results")

        if power > 0.95:
            recommendations.append("Power is very high - consider reducing sample size for efficiency")
