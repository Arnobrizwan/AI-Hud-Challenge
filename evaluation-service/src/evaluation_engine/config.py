...
class Settings(BaseSettings):
    ...
    MLFLOW_EXPERIMENT_NAME: str = Field(
        default="evaluation-suite", env="MLFLOW_EXPERIMENT_NAME"
    )
    ...
    GRAFANA_ENDPOINT: str = Field(
        default="http://localhost:3000", env="GRAFANA_ENDPOINT"
    )
    
    # Alerting settings
    ALERT_WEBHOOK_URL: Optional[str] = Field(
        default=None, env="ALERT_WEBHOOK_URL"
    )
    SLACK_WEBHOOK_URL: Optional[str] = Field(
        default=None, env="SLACK_WEBHOOK_URL"
    )
    EMAIL_SMTP_HOST: Optional[str] = Field(
        default=None, env="EMAIL_SMTP_HOST"
    )
    EMAIL_SMTP_PORT: int = Field(default=587, env="EMAIL_SMTP_PORT")
    EMAIL_SMTP_USER: Optional[str] = Field(
        default=None, env="EMAIL_SMTP_USER"
    )
    ...
    MAX_CONCURRENT_EVALUATIONS: int = Field(
        default=10, env="MAX_CONCURRENT_EVALUATIONS"
    )
    EVALUATION_TIMEOUT_SECONDS: int = Field(
        default=3600, env="EVALUATION_TIMEOUT_SECONDS"
    )
    ...
    DRIFT_SIGNIFICANCE_LEVEL: float = Field(
        default=0.05, env="DRIFT_SIGNIFICANCE_LEVEL"
    )
    DRIFT_ALERT_THRESHOLD: float = Field(
        default=0.7, env="DRIFT_ALERT_THRESHOLD"
    )
    DRIFT_WINDOW_SIZE: int = Field(default=1000, env="DRIFT_WINDOW_SIZE")
    
    # A/B testing settings
    MIN_EXPERIMENT_DURATION_DAYS: int = Field(
        default=7, env="MIN_EXPERIMENT_DURATION_DAYS"
    )
    MAX_EXPERIMENT_DURATION_DAYS: int = Field(
        default=30, env="MAX_EXPERIMENT_DURATION_DAYS"
    )
    MIN_SAMPLE_SIZE_PER_VARIANT: int = Field(
        default=1000, env="MIN_SAMPLE_SIZE_PER_VARIANT"
    )
