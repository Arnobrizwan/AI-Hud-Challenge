...
from safety_engine.models import SafetyIncident, IncidentClassification, EscalationLevel
...
class EscalationManager:
    ...
    async def escalate_incident(self, incident: SafetyIncident, classification: IncidentClassification) -> bool:
        ...
        try:
            ...
            escalation_id = f"escalation_{incident.id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            escalation_record = {
            ...
            await self.send_immediate_notification(incident, classification, escalation_config['immediate_level'])
            
            # Schedule next escalation
            await self.schedule_next_escalation(incident.id, escalation_config)
            
            logger.info(f"Incident {incident.id} escalated to {escalation_config['immediate_level']}")
    ...
    async def send_immediate_notification(self, incident: SafetyIncident, 
    ...
    async def schedule_next_escalation(self, incident_id: str, escalation_config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            current_index = escalation_chain.index(current_level) if current_level in escalation_chain else 0
            
            # Check if there's a next level
            if current_index + 1 < len(escalation_chain):
                next_level = escalation_chain[current_index + 1]
                interval_minutes = escalation_intervals[current_index] if current_index < len(escalation_intervals) else 60
                
                # Schedule next escalation
                asyncio.create_task(self.delayed_escalation(incident_id, next_level, interval_minutes))
                
        except Exception as e:
            logger.error(f"Next escalation scheduling failed: {str(e)}")
    
    async def delayed_escalation(self, incident_id: str, next_level: str, delay_minutes: int) -> None:
        ...
        try:
            ...
            if level_config:
                ...
                await self.send_escalation_notification(level_config['contacts'], message, incident_id, next_level)
    ...
    async def send_escalation_notification(self, contacts: List[str], message: str, 
                                         incident_id: str, level: str) -> bool:
        ...
        try:
            ...
            for channel in level_config['channels']:
                if channel in self.notification_channels:
                    try:
                        result = await self.notification_channels[channel](contacts, message, incident_id)
                        notification_results.append(result)
                    except Exception as e:
    ...
    def create_escalation_message(self, incident: Optional[SafetyIncident], 
...
This incident has been escalated to your level and requires immediate attention.
        ...
        while True:
            try:
                ...
                for incident_id, escalation in self.active_escalations.items():
                    ...
                    if (current_time - escalation['escalation_start']).total_seconds() > 24 * 3600:  # 24 hours
                        stale_escalations.append(incident_id)
                
                # Handle stale escalations
                for incident_id in stale_escalations:
                    logger.warning(f"Escalation for incident {incident_id} has been active for over 24 hours")
    ...
    async def send_email_notification(self, contacts: List[str], message: str, incident_id: str) -> bool:
    ...
    async def send_slack_notification(self, contacts: List[str], message: str, incident_id: str) -> bool:
    ...
    async def send_phone_notification(self, contacts: List[str], message: str, incident_id: str) -> bool:
    ...
    async def send_sms_notification(self, contacts: List[str], message: str, incident_id: str) -> bool:
