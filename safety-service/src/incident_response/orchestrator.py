...
from safety_engine.models import ResponseAction, ResponseResult, ActionStatus
...
class ResponseOrchestrator:
    ...
    async def get_action_statistics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'total_actions': len(self.active_actions) + len(self.completed_actions) + len(self.failed_actions),
    ...
    def calculate_average_execution_time(self) -> float:
        ...
        try:
            execution_times = [
                result.execution_time for result in self.completed_actions.values()
    ...
    async def action_cleanup_task(self):
        ...
        while True:
            try:
                ...
                old_actions = [
                    action_id for action_id, result in self.completed_actions.items()
                ...
                old_actions = [
                    action_id for action_id, result in self.failed_actions.items()
                    if result.completed_at and result.completed_at < cutoff_date
                ]
