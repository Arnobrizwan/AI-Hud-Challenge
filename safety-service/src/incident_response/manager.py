...
from safety_engine.models import (
    SafetyIncident, IncidentResponse, IncidentClassification,
    ResponsePlan, ResponseAction, TimelineItem, MonitoringConfig
)
...
class IncidentResponseManager:
    ...
    async def handle_safety_incident(self, incident: SafetyIncident) -> IncidentResponse:
        ...
        try:
            ...
            logger.info(f"Incident {incident.id} handled with response plan {response_plan.plan_id}")
    ...
    async def generate_response_plan(self, incident: SafetyIncident, 
                                   classification: IncidentClassification) -> ResponsePlan:
    ...
    async def generate_response_actions(self, incident: SafetyIncident, 
                                      classification: IncidentClassification) -> List[ResponseAction]:
    ...
    async def generate_response_timeline(self, incident: SafetyIncident, 
                                       classification: IncidentClassification) -> List[TimelineItem]:
    ...
    async def generate_success_criteria(self, incident: SafetyIncident, 
                                      classification: IncidentClassification) -> List[str]:
    ...
    async def cleanup_resolved_incidents(self):
        ...
        try:
            ...
            old_incidents = [
                incident_id for incident_id, incident in self.resolved_incidents.items()
                ...
                for incident in self.active_incidents.values():
                    if incident.status == 'open':
                        ...
                        if incident.detected_at < datetime.utcnow() - timedelta(hours=24):
                            logger.warning(f"Incident {incident.id} has been open for over 24 hours")
    ...
    def calculate_average_resolution_time(self) -> float:
        ...
        try:
            resolved_incidents = [incident for incident in self.resolved_incidents.values() 
