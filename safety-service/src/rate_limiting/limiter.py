...
from .strategies import (
    ...
    AdaptiveRateLimiter, GeolocationBasedLimiter
)
...
class AdvancedRateLimiter:
    ...
    async def check_rate_limits(self, request: RateLimitRequest) -> RateLimitResult:
        ...
        try:
            ...
            limit_checks = await asyncio.gather(
                ...
                self.adaptive_limiter.check_limit(user_id, endpoint, request.current_load, reputation_score, abuse_score),
            ...
            if not is_rate_limited:
                await self.apply_dynamic_rate_limit(user_id, endpoint, reputation_score, abuse_score)
    ...
    async def apply_dynamic_rate_limit(self, user_id: str, endpoint: str, 
                                     reputation_score: float, abuse_score: float):
    ...
    async def get_system_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_names = ['sliding_window', 'token_bucket', 'adaptive_limiter', 'geolocation_limiter']
    ...
    async def apply_rate_limit_penalty(self, user_id: str, penalty_multiplier: float, duration_minutes: int = 60):
        ...
        try:
            ...
            logger.info(f"Applied rate limit penalty to user {user_id}: {penalty_multiplier}x for {duration_minutes} minutes")
