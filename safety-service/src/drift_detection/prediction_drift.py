...
class PredictionDriftDetector:
    ...
    async def detect_prediction_drift(self, reference_predictions: pd.Series,
                                    current_predictions: pd.Series) -> PredictionDriftResult:
        """Detect drift in model predictions"""
        ...
        try:
            ...
            accuracy_change = self.calculate_accuracy_change(reference_predictions, current_predictions)
    ...
    async def detect_prediction_distribution_drift(self, ref_pred: pd.Series,
        ...
        try:
            ...
            # Mann-Whitney U test
            mw_statistic, mw_p_value = stats.mannwhitneyu(ref_pred, curr_pred, alternative='two-sided')
    ...
    def calculate_calibration_curve(self, probabilities: np.ndarray, n_bins: int = 10) -> np.ndarray:
