...
class ConceptDriftDetector:
    ...
    async def detect_concept_drift(
        ...
        try:
            ...
            if ref_data is None or curr_data is None:
                return ConceptDriftResult(drift_detected=False, drift_score=0.0, affected_features=[], confidence=1.0)
            ...
            return ConceptDriftResult(drift_detected=False, drift_score=0.0, affected_features=[], confidence=0.0)

    def prepare_data(self, reference_labels: pd.Series, current_labels: pd.Series, feature_data: pd.DataFrame) -> tuple:
        """Prepare data for concept drift detection"""
        try:
            # Ensure we have enough data
            if len(reference_labels) < self.config.min_samples or len(current_labels) < self.config.min_samples:
            ...
            # Get common features
            ref_features = feature_data.loc[ref_indices] if ref_indices.isin(feature_data.index).any() else None
            curr_features = feature_data.loc[curr_indices] if curr_indices.isin(feature_data.index).any() else None
    ...
    def identify_affected_features(self, ref_data: Dict, curr_data: Dict) -> List[str]:
        ...
        try:
            ...
            affected_features = [feature for feature, change in feature_importance_pairs if change > threshold]
