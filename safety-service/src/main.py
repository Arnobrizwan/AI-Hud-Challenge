...
from safety_engine.models import (
    SafetyMonitoringRequest, SafetyStatus, DriftDetectionRequest,
    AbuseDetectionRequest, ContentModerationRequest, RateLimitRequest,
    ComplianceRequest, IncidentResponse
)
from safety_engine.config import get_settings
from safety_engine.database import init_database, get_db_session
from safety_engine.middleware import SafetyMiddleware, RateLimitMiddleware
from safety_engine.monitoring import setup_monitoring, get_metrics
...
# Prometheus metrics
safety_checks_total = Counter('safety_checks_total', 'Total safety checks performed', ['check_type', 'status'])
safety_check_duration = Histogram('safety_check_duration_seconds', 'Time spent on safety checks', ['check_type'])
active_incidents = Gauge('active_incidents_total', 'Number of active safety incidents', ['severity'])
drift_detection_score = Gauge('drift_detection_score', 'Current drift detection score')
abuse_detection_score = Gauge('abuse_detection_score', 'Current abuse detection score')
...
async def monitor_system_safety(
    ...
    try:
        safety_checks_total.labels(check_type="comprehensive", status="started").inc()
        ...
        # Update metrics
        drift_detection_score.set(safety_status.drift_status.overall_drift_score if safety_status.drift_status else 0.0)
        abuse_detection_score.set(safety_status.abuse_status.abuse_score if safety_status.abuse_status else 0.0)
        
        safety_checks_total.labels(check_type="comprehensive", status="completed").inc()
        
        # Trigger background response if needed
        if safety_status.requires_intervention:
            background_tasks.add_task(handle_safety_intervention, safety_status)
        
        return safety_status
        
    except Exception as e:
        safety_checks_total.labels(check_type="comprehensive", status="error").inc()
...
async def detect_drift(request: DriftDetectionRequest):
    ...
    try:
        ...
        safety_checks_total.labels(check_type="drift", status="completed").inc()
        return drift_result
...
async def detect_abuse(request: AbuseDetectionRequest):
    ...
    try:
        ...
        safety_checks_total.labels(check_type="abuse", status="completed").inc()
        return abuse_result
...
async def moderate_content(request: ContentModerationRequest):
    ...
    try:
        safety_checks_total.labels(check_type="content", status="started").inc()
        
        with safety_check_duration.labels(check_type="content").time():
            moderation_result = await safety_engine.content_moderator.moderate_content(request.content)
        
        safety_checks_total.labels(check_type="content", status="completed").inc()
...
async def check_rate_limits(request: RateLimitRequest):
    ...
    try:
        safety_checks_total.labels(check_type="rate_limit", status="started").inc()
        ...
        safety_checks_total.labels(check_type="rate_limit", status="completed").inc()
        return rate_limit_result
        
    except Exception as e:
        safety_checks_total.labels(check_type="rate_limit", status="error").inc()
        logger.error(f"Rate limiting check failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Rate limiting check failed: {str(e)}")

@app.post("/safety/compliance/check")
async def check_compliance(request: ComplianceRequest):
    """Compliance monitoring endpoint"""
    try:
        safety_checks_total.labels(check_type="compliance", status="started").inc()
        ...
        safety_checks_total.labels(check_type="compliance", status="completed").inc()
        return compliance_result
        
    except Exception as e:
        safety_checks_total.labels(check_type="compliance", status="error").inc()
...
async def handle_safety_intervention(safety_status: SafetyStatus):
    """Handle safety intervention in background"""
    try:
        await safety_engine.trigger_safety_response(safety_status)
        logger.info(f"Safety intervention triggered for status: {safety_status.overall_score}")
    except Exception as e:
