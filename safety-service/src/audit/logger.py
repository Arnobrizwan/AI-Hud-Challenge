...
from .models import AuditEvent, AuditLog, AuditReport, AuditQuery, AuditMetrics
...
class AuditLogger:
    ...
    async def log_safety_check(self, request: SafetyMonitoringRequest, 
        ...
        try:
            event = AuditEvent(
                ...
                details={
                    ...
                    'content_status': status.content_status.__dict__ if status.content_status else None
                },
        ...
        try:
            event = AuditEvent(
                ...
                details={
                    ...
                    'drifted_features': drift_result.data_drift.drifted_features if drift_result.data_drift else []
                },
        ...
        try:
            event = AuditEvent(
                ...
                details={
                    ...
                    'rule_violations': len(abuse_result.rule_violations) if abuse_result.rule_violations else 0
                },
        ...
        try:
            event = AuditEvent(
                ...
                details={
                    ...
                    'violations': len(moderation_result.violations) if moderation_result.violations else 0,
        ...
        try:
            event = AuditEvent(
                ...
                details={
                    ...
                    'violations': len(compliance_result.violations) if compliance_result.violations else 0,
    ...
    async def query_audit_events(self, query: AuditQuery) -> List[AuditEvent]:
        ...
        try:
            ...
            if query.sort_by == "timestamp":
                ...
                events.sort(key=lambda x: severity_order.get(x.severity, 0), reverse=(query.sort_order == "desc"))
    ...
    async def generate_audit_report(self, start_time: datetime, end_time: datetime) -> AuditReport:
        ...
        try:
            ...
            top_users = [{"user_id": user, "count": count} for user, count in sorted(user_counts.items(), key=lambda x: x[1], reverse=True)[:10]]
            ...
            top_resources = [{"resource": resource, "count": count} for resource, count in sorted(resource_counts.items(), key=lambda x: x[1], reverse=True)[:10]]
            ...
            top_actions = [{"action": action, "count": count} for action, count in sorted(action_counts.items(), key=lambda x: x[1], reverse=True)[:10]]
            
            # Get security and compliance events
            security_events = [e for e in events if e.event_type in ["abuse_detection", "rate_limiting", "system_access"]]
            compliance_events = [e for e in events if e.event_type in ["compliance_check", "data_access"]]
    ...
    async def get_audit_metrics(self) -> AuditMetrics:
        ...
        try:
            ...
            top_event_types = [{"type": event_type, "count": count} for event_type, count in sorted(event_type_counts.items(), key=lambda x: x[1], reverse=True)[:10]]
            ...
            security_events_count = len([e for e in self.audit_events if e.event_type in ["abuse_detection", "rate_limiting", "system_access"]])
            compliance_events_count = len([e for e in self.audit_events if e.event_type in ["compliance_check", "data_access"]])
        ...
        while True:
            try:
                ...
                if old_events or old_logs:
                    logger.info(f"Cleaned up {len(old_events)} old events and {len(old_logs)} old logs")
