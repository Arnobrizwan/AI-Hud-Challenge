...
from safety_engine.models import AnomalyDetectionRequest, AnomalyDetectionResult, Anomaly
from safety_engine.config import get_anomaly_config
from .detectors import (
    IsolationForestDetector, OneClassSVMDetector,
    LSTMAutoencoderDetector, StatisticalAnomalyDetector
)
...
class AnomalyDetectionSystem:
    ...
    def generate_recommendations(self, anomalies: List[Anomaly], system_health: str) -> List[str]:
        ...
        try:
            ...
            if system_health == 'critical':
                recommendations.append("Immediate intervention required - system is in critical state")
            ...
            for anomaly in anomalies:
                if anomaly.metric_name == 'cpu_usage' and anomaly.value > 0.8:
                    ...
                    recommendations.append("High response time detected - optimize database queries or caching")
                elif anomaly.metric_name == 'request_rate' and anomaly.value > 1000:
                    recommendations.append("High request rate detected - consider rate limiting or scaling")
    ...
    async def update_historical_data(self, request: AnomalyDetectionRequest, anomaly_score: float):
        ...
        try:
            ...
            self.historical_metrics = [
                data for data in self.historical_metrics
            ...
            if request.user_behavior:
                ...
                if user_id:
                    ...
                    cutoff_time = datetime.utcnow() - timedelta(hours=24)
                    self.user_behavior_history[user_id] = [
                        data for data in self.user_behavior_history[user_id]
                        if data['timestamp'] > cutoff_time
                    ]
    ...
    def extract_features_from_historical_data(self, data: Dict[str, Any]) -> List[float]:
        ...
        try:
            ...
            for metric_name in ['cpu_usage', 'memory_usage', 'disk_usage', 'network_usage', 'error_rate', 'response_time']:
                features.append(system_metrics.get(metric_name, 0.0))
            
            # User behavior metrics
            user_behavior = data.get('user_behavior', {})
            for behavior_name in ['activity_level', 'request_frequency', 'session_duration', 'error_count']:
