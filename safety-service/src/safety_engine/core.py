...
class SafetyMonitoringEngine:
    ...
    async def monitor_system_safety(self, monitoring_request: SafetyMonitoringRequest) -> SafetyStatus:
    ...
    async def perform_safety_checks(self, request: SafetyMonitoringRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            checks['user_behavior'] = await self.analyze_user_behavior(request.user_id, request.features)
    ...
    async def trigger_safety_response(self, safety_status: SafetyStatus) -> None:
        ...
        try:
            if safety_status.requires_intervention:
                logger.warning(f"Safety intervention required: overall_score={safety_status.overall_score}")
    ...
    async def log_safety_check(self, request: SafetyMonitoringRequest, status: SafetyStatus) -> None:
        ...
        try:
            ...
            logger.info(f"Safety check logged: user_id={request.user_id}, score={status.overall_score}")
