...
class ReputationSystem:
    ...
    async def update_reputation(self, user_id: str, event_type: str, 
                              event_data: Optional[Dict[str, Any]] = None) -> float:
        ...
        try:
            ...
            logger.info(f"Updated reputation for user {user_id}: {current_reputation:.3f} -> {new_reputation:.3f} ({event_type})")
    ...
    def calculate_reputation_change(self, event_type: str, event_data: Optional[Dict[str, Any]]) -> float:
    ...
    async def get_reputation_trend(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_events = [
                event for event in self.reputation_events.get(user_id, [])
            ...
            changes = [recent_reputations[i+1] - recent_reputations[i] for i in range(len(recent_reputations)-1)]
    ...
    async def get_top_users(self, limit: int = 10, min_reputation: float = 0.0) -> List[Dict[str, Any]]:
        """Get top users by reputation"""
        try:
            # Filter users by minimum reputation
            filtered_users = {
                user_id: reputation for user_id, reputation in self.user_reputations.items()
        ...
        while True:
            try:
                ...
                for user_id in list(self.user_reputations.keys()):
                    ...
                    if abs(decayed_reputation - current_reputation) > 0.001:  # Only update if significant change
