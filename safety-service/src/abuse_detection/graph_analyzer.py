...
class GraphBasedAbuseDetector:
    ...
    async def analyze_user_graph(self, user_id: str, connection_data: Optional[Dict[str, Any]]) -> GraphSignals:
    ...
    def calculate_abuse_probability(self, suspicious_connections: int, 
                                  cluster_anomaly: bool, centrality_anomaly: bool) -> float:
    ...
    async def identify_abuse_clusters(self) -> List[List[str]]:
        ...
        try:
            ...
            abuse_clusters = [cluster for label, cluster in clusters.items() 
                            if label != -1 and len(cluster) > 2]
