...
class ExternalModerationAPIs:
    ...
    async def call_api(
        self, api_name: str, content_type: str, content: str
    ...
    async def call_google_perspective(
        self, config: Dict[str, Any], content_type: str, content: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.post(
                config["endpoint"], json=request_data, headers=headers, timeout=config["timeout"]
            ) as response:
    ...
    async def call_microsoft_content_moderator(
        self, config: Dict[str, Any], content_type: str, content: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Make API call
            async with self.session.post(
                config["endpoint"], data=content, headers=headers, timeout=config["timeout"]
            ) as response:
                if response.status == 200:
                    result = await response.json()
    ...
    async def call_azure_content_safety(
        self, config: Dict[str, Any], content_type: str, content: str
    ) -> Optional[Dict[str, Any]]:
