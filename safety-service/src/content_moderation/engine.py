...
from safety_engine.models import (
    ContentModerationRequest, ModerationResult, TextModerationResult,
    ContentViolation, ModerationAction
)
from safety_engine.config import get_content_config
from .detectors import (
    ToxicityDetector, HateSpeechDetector, SpamDetector,
    MisinformationDetector, AdultContentDetector, ViolenceDetector
)
...
class ContentModerationEngine:
    ...
    async def moderate_content(self, content: Any) -> ModerationResult:
        ...
        try:
            ...
            moderation_action = self.determine_moderation_action(overall_safety_score, moderation_results)
    ...
    async def moderate_text_content(self, text: str) -> TextModerationResult:
        ...
        try:
            ...
            toxicity_result, hate_speech_result, spam_result, misinformation_result = moderation_checks
            ...
            toxicity_score = toxicity_result.toxicity_score if not isinstance(toxicity_result, Exception) else 0.0
            hate_speech_score = hate_speech_result.hate_score if not isinstance(hate_speech_result, Exception) else 0.0
            spam_score = spam_result.spam_score if not isinstance(spam_result, Exception) else 0.0
            misinformation_score = misinformation_result.misinformation_score if not isinstance(misinformation_result, Exception) else 0.0
    ...
    async def moderate_image_content(self, image_urls: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, image_url in enumerate(image_urls):
                ...
                # Extract scores
                adult_score = adult_result.adult_score if not isinstance(adult_result, Exception) else 0.0
                violence_score = violence_result.violence_score if not isinstance(violence_result, Exception) else 0.0
                
                image_results[f"image_{i}"] = {
                    'url': image_url,
                    'adult_content_score': adult_score,
                    'violence_score': violence_score,
                    'is_safe': adult_score < self.config.adult_content_threshold and violence_score < self.config.violence_threshold
                }
    ...
    def determine_moderation_action(self, safety_score: float, moderation_results: Dict[str, Any]) -> ModerationAction:
    ...
    def extract_violations(self, moderation_results: Dict[str, Any]) -> List[ContentViolation]:
        ...
        try:
            ...
            # Media violations
            if 'images' in moderation_results:
                violations.extend(self.extract_media_violations(moderation_results['images'], 'image'))
            
            if 'videos' in moderation_results:
                violations.extend(self.extract_media_violations(moderation_results['videos'], 'video'))
        ...
        try:
            ...
            if text_result.toxicity_score > self.config.toxicity_threshold:
                violations.append(ContentViolation(
                    ...
                    affected_content=text_result.text[:100] + '...' if len(text_result.text) > 100 else text_result.text
                ))
    ...
    def extract_media_violations(self, media_results: Dict[str, Any], media_type: str) -> List[ContentViolation]:
