...
class GDPRComplianceMonitor:
    ...
    async def check_gdpr_compliance(self, data_processing_activities: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    async def check_activity_compliance(self, activity: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        try:
            ...
            if activity.get('retention_period') and activity.get('retention_period') > self.config.data_retention_period:
                violations.append(ComplianceViolation(
                    violation_type=ComplianceType.GDPR,
                    severity='high',
        ...
        try:
            ...
            for violation in violations:
                if "lawful basis" in violation.description.lower():
                    recommendations.append("Ensure all data processing activities have a valid lawful basis under GDPR Article 6")
