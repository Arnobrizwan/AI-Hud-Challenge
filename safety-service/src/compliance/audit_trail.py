...
class AuditTrailManager:
    ...
    async def log_content_moderation(
        ...
        try:
            return await self.log_event(
                ...
                result=(
                    "success"
                    if getattr(moderation_result, "overall_safety_score", 0) > 0.8
                    else "warning"
                ),
            )
    ...
    async def log_rate_limiting(
        ...
        try:
            return await self.log_event(
                ...
                result=(
                    "success"
                    if not getattr(rate_limit_result, "is_rate_limited", False)
                    else "warning"
                ),
            )
    ...
    async def log_compliance_check(self, request: Any, report: Any) -> str:
        ...
        try:
            return await self.log_event(
                ...
                result=(
                    "success" if getattr(report, "overall_compliance_score", 0) > 0.8 else "warning"
                ),
            )
    ...
    async def log_data_access(
        self, user_id: str, resource: str, action: str, details: Dict[str, Any]
    ) -> str:
