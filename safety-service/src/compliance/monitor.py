...
from safety_engine.models import ComplianceRequest, ComplianceReport, ComplianceViolation, ComplianceType
...
class ComplianceMonitor:
    ...
    def extract_compliance_violations(self, compliance_results: Dict[str, Any]) -> List[ComplianceViolation]:
    ...
    async def generate_compliance_recommendations(self, compliance_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if overall_score < 0.8:
                recommendations.append("Overall compliance score is below 80% - review all compliance areas")
            
            if overall_score < 0.6:
                recommendations.append("Critical compliance issues detected - immediate action required")
    ...
    async def generate_compliance_report(self, report_type: str = 'summary') -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.gdpr_enabled:
                report['detailed_results']['gdpr'] = await self.gdpr_monitor.generate_detailed_report()
            
            if self.config.content_policy_enabled:
                report['detailed_results']['content_policy'] = await self.content_policy_monitor.generate_detailed_report()
            
            if self.config.privacy_enabled:
                report['detailed_results']['privacy'] = await self.privacy_monitor.generate_detailed_report()
