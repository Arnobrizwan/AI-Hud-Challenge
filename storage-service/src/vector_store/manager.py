...
from models import (
    SimilaritySearchParams, SimilaritySearchResult, SimilarityResult,
    VectorStorageResult, VectorIndexConfig, IndexBuildResult,
    SearchMethod
)
...
class VectorStoreManager:
    ...
    async def store_embeddings(self, content_id: str, 
                             embeddings: Dict[str, List[float]]) -> VectorStorageResult:
    ...
    async def similarity_search(self, search_params: SimilaritySearchParams) -> SimilaritySearchResult:
        ...
        try:
            ...
            logger.info(f"Similarity search completed in {search_duration}ms, found {len(results)} results")
    ...
    async def _exact_similarity_search(self, query_vector: np.ndarray,
                                     params: SimilaritySearchParams) -> List[SimilarityResult]:
    ...
    async def _approximate_similarity_search(self, query_vector: np.ndarray,
                                           params: SimilaritySearchParams) -> List[SimilarityResult]:
    ...
    async def _hybrid_similarity_search(self, query_vector: np.ndarray,
                                      params: SimilaritySearchParams) -> List[SimilarityResult]:
        """Hybrid similarity search combining exact and approximate methods"""
        try:
            # Get approximate results first (faster)
            approximate_results = await self._approximate_similarity_search(
                query_vector, params
            )
            ...
            exact_results = await self._exact_similarity_search(
                query_vector, exact_params
            )
    ...
    async def _apply_similarity_filters(self, results: List[SimilarityResult],
