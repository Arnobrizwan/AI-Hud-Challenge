...
from models import Article, IndexingResult, SearchRequest, SearchResult, SearchResultItem
...
class ElasticsearchManager:
    ...
    async def _initialize_index_templates(self) -> Dict[str, Any]:
        ...
        try:
                ...
                "template": {
                    ...
                    "mappings": {
                        "properties": {
                            ...
                            "summary": {"type": "text", "analyzer": "custom_text_analyzer"},
            ...
            await self.es_client.indices.put_index_template(
                name="articles_template", body=article_template
            )
    ...
    async def index_article(self, article: Article) -> IndexingResult:
        ...
        try:
            ...
            result = await self.es_client.index(
                index=index_name, id=article.id, body=doc, refresh="wait_for"
            )
    ...
    async def search_articles(
        ...
        try:
            ...
            response = await self.es_client.search(index=indices, body=search_body)
    ...
    async def get_article_content(self, article_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.es_client.search(
                index="articles-*", body={"query": {"term": {"id": article_id}}}
            )
    ...
    """Build complex search query"""
            ...
            query_parts.append(
                {
                    "multi_match": {
                        ...
                        "fields": ["title^2.0", "summary^1.5", "content^1.0"],
                        "type": "best_fields",
                        "fuzziness": "AUTO",
                    }
                }
        ...
        if search_request.languages:
            filters.append({"terms": {"language": search_request.languages}})

        if search_request.date_range:
            date_filter = {"range": {"published_at": {}}}
            if search_request.date_range.get("start"):
                date_filter["range"]["published_at"]["gte"] = search_request.date_range[
                    "start"
                ].isoformat()
            if search_request.date_range.get("end"):
                date_filter["range"]["published_at"]["lte"] = search_request.date_range[
                    "end"
                ].isoformat()
