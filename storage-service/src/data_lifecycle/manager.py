...
from models import (
    GDPRRequest, GDPRResponse, GDPRDeletionResult, RetentionResult,
...
class DataLifecycleManager:
        ...
        try:
            ...
            if self.archival_service:
                ...
                deletion_results.append({
                    'store': 'postgresql',
                    'deleted_count': pg_result.get('deleted_count', 0)
                })
            ...
            if self.archival_service:
                ...
                deletion_results.append({
                    'store': 'elasticsearch',
                    'deleted_count': es_result.get('deleted_count', 0)
                })
            ...
            if self.archival_service:
                ...
                deletion_results.append({
                    'store': 'redis',
                    'deleted_count': redis_result.get('deleted_count', 0)
                })
    ...
    async def handle_gdpr_request(self, gdpr_request: GDPRRequest) -> GDPRResponse:
        ...
        logger.info(f"Handling GDPR request {gdpr_request.request_id} for user {gdpr_request.user_id}")
    ...
    async def log_gdpr_request(self, gdpr_request: GDPRRequest):
        ...
        try:
            ...
            logger.info(f"GDPR request logged: {gdpr_request.request_id} - {gdpr_request.request_type} - {gdpr_request.user_id}")
