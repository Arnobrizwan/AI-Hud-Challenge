...
from models import (
    ...
    SimilaritySearchParams, SimilaritySearchResult, RetrievalOptions,
    StorageType, SimilarityResult
)
...
class StorageOrchestrator:
    ...
    async def retrieve_article(self, article_id: str, 
                             retrieval_options: RetrievalOptions) -> RetrievedArticle:
    ...
    async def similarity_search(self, search_params: SimilaritySearchParams) -> SimilaritySearchResult:
    ...
    def _create_storage_task(self, store_type: StorageType, operation: str, data: Any) -> StorageTask:
    ...
    async def _create_retrieval_plan(self, article_id: str, 
                                   retrieval_options: RetrievalOptions) -> List[Dict[str, Any]]:
        """Create optimal retrieval plan"""
        plan = []
        
        # Always try PostgreSQL first for metadata
        plan.append({
            'store_type': StorageType.POSTGRESQL,
            'priority': 1,
            'required': True
        })
        ...
        plan.append({
            'store_type': StorageType.ELASTICSEARCH,
            'priority': 2,
            'required': False
        })
        
        # Add vector store if embeddings needed
        if retrieval_options.preferred_sources and StorageType.VECTOR_STORE in retrieval_options.preferred_sources:
            plan.append({
                'store_type': StorageType.VECTOR_STORE,
                'priority': 3,
                'required': False
            })
        
        # Add media storage if media needed
        if retrieval_options.preferred_sources and StorageType.MEDIA_STORAGE in retrieval_options.preferred_sources:
            plan.append({
                'store_type': StorageType.MEDIA_STORAGE,
                'priority': 4,
                'required': False
            })
    ...
    def _combine_article_data(self, article_id: str, article_data: Dict[str, Any]) -> RetrievedArticle:
