...
class PersonalizationEngine:
    ...
    async def personalize_ranking(
        ...
        try:
            ...
            for article in articles:
                ...
                cf_score = await self.collaborative_filter.predict(user_id, article.id)
                ...
                geo_score = await self._compute_geographic_preference(article, user_profile)
    ...
    async def _compute_source_preference(
        self, source_id: str, source_preferences: Dict[str, float]
    ) -> float:
        """Compute source preference score."""
        return source_preferences.get(source_id, 0.5)

    async def _compute_time_preference(
        self, published_at: datetime, reading_patterns: Dict[str, Any]
    ) -> float:
    ...
    async def _compute_geographic_preference(
        self, article: Article, user_profile: UserProfile
    ) -> float:
    ...
    async def _update_reading_patterns(
        ...
        if reading_time > 60:  # If user spent more than 1 minute reading
            if hour not in profile.reading_patterns["preferred_hours"]:
                ...
                profile.reading_patterns["preferred_hours"] = profile.reading_patterns[
                    "preferred_hours"
                ][-10:]
    ...
    async def compute_similarity(
        ...
        try:
            ...
            if "preferred_length" in user_preferences:
                ...
                length_similarity = 1.0 - abs(preferred_length - actual_length) / max(
                    preferred_length, actual_length
                )
