...
class RankingMetricsCollector(MetricsCollector):
    ...
    async def record_ranking(self, response_time_ms: float, feature_time_ms: float,
                           ranking_time_ms: float, cache_hit_rate: float,
                           article_count: int, user_id: Optional[str] = None,
                           algorithm_variant: Optional[str] = None):
        ...
        try:
            ...
            self.response_time_histogram.labels(algorithm_variant=variant).observe(response_time_ms / 1000)
            self.feature_time_histogram.observe(feature_time_ms / 1000)
            self.ranking_time_histogram.labels(algorithm_variant=variant).observe(ranking_time_ms / 1000)
...
class SystemMetricsCollector(MetricsCollector):
    ...
    async def record_system_metrics(self, cpu_usage: float, memory_usage: float,
                                  redis_connections: int, active_requests: int,
                                  queue_size: int):
