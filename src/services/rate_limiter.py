...
class RateLimitExceeded(Exception):
    ...
    def __init__(self, limit_type: str, limit: int, window_seconds: int, reset_time: datetime):
        ...
        super().__init__(f"Rate limit exceeded for {limit_type}: {limit} requests per {window_seconds}s")
...
class SlidingWindowRateLimiter:
    ...
    async def check_rate_limit(self, key: str, limit: int, 
                             window_seconds: int = 60) -> Tuple[bool, RateLimitInfo]:
        """
    ...
    async def get_rate_limit_info(self, key: str, limit: int,
...
class DistributedRateLimiter:
    ...
    async def check_user_rate_limit(self, user_id: str, 
                                  limit: Optional[int] = None,
                                  window_seconds: Optional[int] = None) -> Tuple[bool, RateLimitInfo]:
    ...
    async def check_endpoint_rate_limit(self, endpoint: str, identifier: str,
                                      limit: int, window_seconds: int = 60) -> Tuple[bool, RateLimitInfo]:
        """Check rate limit for a specific endpoint."""
        key = f"endpoint:{endpoint}:{identifier}"
        return await self.limiter.check_rate_limit(key, limit, window_seconds)
    
    async def check_global_rate_limit(self, limit: int = 100000, 
                                    window_seconds: int = 60) -> Tuple[bool, RateLimitInfo]:
    ...
    async def get_user_rate_limit_info(self, user_id: str,
                                     limit: Optional[int] = None,
    ...
    async def get_ip_rate_limit_info(self, ip_address: str,
                                   limit: Optional[int] = None,
                                   window_seconds: Optional[int] = None) -> RateLimitInfo:
