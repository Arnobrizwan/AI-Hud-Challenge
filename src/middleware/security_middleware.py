...
from src.utils.logging import get_logger, set_correlation_id, get_correlation_id
...
class SecurityMiddlewareStack:
    ...
    def create_stack(app: ASGIApp, custom_config: Optional[Dict] = None) -> ASGIApp:
        ...
        app = RequestSizeLimitMiddleware(
            app,
            max_size=config.get('max_request_size', settings.MAX_REQUEST_SIZE)
        )
        
        # 5. Request timeout
        app = RequestTimeoutMiddleware(
            app,
            timeout_seconds=config.get('request_timeout', settings.REQUEST_TIMEOUT)
        )
