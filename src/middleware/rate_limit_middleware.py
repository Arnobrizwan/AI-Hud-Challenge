...
from src.services.rate_limiter import rate_limiter, RateLimitExceeded
from src.models.common import RateLimitResponse, RateLimitInfo, ResponseStatus
...
class RateLimitMiddleware(BaseHTTPMiddleware):
        ...
        try:
            ...
            for check_name, (allowed, rate_limit_info) in results.items():
                if not allowed and rate_limit_info:
                    ...
                    elif check_name == 'ip_limit' and most_restrictive_type not in ['endpoint', 'user']:
                        most_restrictive_info = rate_limit_info
            ...
            if most_restrictive_info:
                ...
                return self._create_rate_limit_response(most_restrictive_info, most_restrictive_type)
    ...
    def _create_rate_limit_response(self, rate_limit_info: RateLimitInfo, limit_type: str) -> JSONResponse:
...
class AdaptiveRateLimitMiddleware(RateLimitMiddleware):
    ...
    async def _is_suspicious_request(self, request: Request, client_ip: str, user_id: Optional[str]) -> bool:
    ...
    async def _is_trusted_request(self, request: Request, client_ip: str, user_id: Optional[str]) -> bool:
    ...
    def _has_suspicious_user_agent(self, request: Request) -> bool:
        ...
        suspicious_patterns = [
            "bot", "crawler", "scraper", "curl", "wget", "python-requests"
        ]
...
async def check_rate_limit_dependency(request: Request, limit: int = 100, window: int = 60):
    ...
    user_id = getattr(request.state, "user", {}).get("uid") if hasattr(request.state, "user") else None
