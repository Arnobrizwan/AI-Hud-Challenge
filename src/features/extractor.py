...
from ..schemas import Article, RankingRequest, TrendingScore, AuthorityScore
...
class RankingFeatureExtractor:
    ...
    async def compute_personalization_features(self, article: Article, 
        ...
        try:
            ...
            if preferred_length > 0:
                length_similarity = 1.0 - abs(preferred_length - article.word_count) / max(preferred_length, article.word_count)
    ...
    async def compute_contextual_features(self, article: Article, 
                                        request: RankingRequest) -> Dict[str, float]:
    ...
    async def compute_interaction_features(self, article: Article, 
                                         user_id: str) -> Dict[str, float]:
    ...
    def _get_default_content_features(self) -> Dict[str, float]:
        ...
        return {
            'title_length': 0, 'content_length': 0, 'word_count': 0, 'reading_time': 0,
            'quality_score': 0.5, 'readability_score': 0.5, 'avg_sentence_length': 0,
            'paragraph_count': 0, 'sentiment_polarity': 0.0, 'sentiment_subjectivity': 0.0,
            'entity_count': 0, 'topic_count': 0, 'unique_topics': 0, 'has_image': 0.0,
            'has_video': 0.0, 'video_count': 0, 'language': 1.0, 'title_caps_ratio': 0.0,
            'title_question_mark': 0.0, 'title_exclamation': 0.0, 'has_summary': 0.0,
            'summary_length': 0
        }
    ...
    def _get_default_freshness_features(self) -> Dict[str, float]:
        ...
        return {
            'age_hours': 24, 'age_days': 1, 'age_log': 3.2, 'age_sqrt': 4.9,
            'freshness_24h': 0.37, 'freshness_6h': 0.02, 'freshness_1h': 0.0,
            'is_breaking': 0.0, 'is_recent': 0.0, 'is_today': 1.0, 'is_this_week': 1.0,
            'is_this_month': 1.0, 'hour_of_day': 12, 'day_of_week': 0, 'is_weekend': 0.0,
            'is_business_hours': 1.0, 'time_engagement_boost': 1.0
        }
    
    def _get_default_authority_features(self) -> Dict[str, float]:
        """Default authority features when extraction fails."""
        return {
            'source_authority': 0.5, 'source_reliability': 0.5, 'source_popularity': 0.5,
            'source_recency': 0.5, 'author_authority': 0.5, 'has_author': 0.0,
            'citation_count': 0, 'social_shares': 0, 'backlink_count': 0,
            'domain_length': 10, 'is_https': 1.0, 'subdomain_count': 0
        }
    
    def _get_default_personalization_features(self) -> Dict[str, float]:
        """Default personalization features when extraction fails."""
        return {
            'avg_topic_affinity': 0.5, 'max_topic_affinity': 0.5, 'topic_affinity_variance': 0.0,
            'source_preference': 0.5, 'length_preference': 0.5, 'time_preference': 0.5,
    ...
    def _get_default_contextual_features(self) -> Dict[str, float]:
        ...
        return {
            'title_query_match': 0.0, 'content_query_match': 0.0, 'query_word_count': 0,
            'title_query_similarity': 0.0, 'geo_distance': 0.0, 'geo_relevance': 0.5,
            'requested_content_type': 1.0, 'topic_filter_match': 0.0, 'source_filter_match': 0.0,
    ...
    def _get_default_interaction_features(self) -> Dict[str, float]:
        ...
        return {
            ...
            'total_engagement': 0, 'engagement_rate': 0, 'like_rate': 0, 'share_rate': 0,
            'comment_rate': 0, 'user_has_viewed': 0.0, 'user_has_liked': 0.0,
            'user_has_shared': 0.0, 'user_has_commented': 0.0, 'trending_score': 0.0,
            'trending_velocity': 0.0, 'trending_acceleration': 0.0
        }
    ...
    async def compute_trending_score(self, article: Article) -> TrendingScore:
        ...
        try:
            ...
            for window_hours in self.time_windows:
                ...
                count = await self._get_article_count_in_window(
                    article.topics, window_hours
                )
                
                # Get baseline count for same period previous week
                baseline = await self._get_baseline_count(
                    article.topics, window_hours
                )
...
class GeographicCalculator:
    ...
    async def compute_distance(self, location1: Dict[str, float], 
