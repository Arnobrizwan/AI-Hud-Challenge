...
class TestFirebaseAuthService:
    ...
    async def firebase_service(self) -> AsyncGenerator[FirebaseAuthService, None]:
    ...
    async def test_validate_token_success(
        self, firebase_service: FirebaseAuthService, mock_firebase_auth: Mock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_validate_token_expired(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_validate_token_invalid(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_validate_token_revoked(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_uid_success(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_uid_not_found(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_revoke_tokens_success(self, firebase_service: FirebaseAuthService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_revoke_tokens_failure(self, firebase_service: FirebaseAuthService) -> None:
    ...
    def test_health_check_success(self, firebase_service: FirebaseAuthService) -> None:
    ...
    def test_health_check_failure(self, firebase_service: FirebaseAuthService) -> None:
...
class TestJWTAuthService:
    ...
    @pytest.fixture
    def jwt_service(self) -> JWTAuthService:
        """Create JWT auth service for testing."""
        return JWTAuthService()

    def test_create_access_token(self, jwt_service: JWTAuthService) -> None:
    ...
    def test_create_access_token_with_custom_expiry(self, jwt_service: JWTAuthService) -> None:
    ...
    def test_verify_token_success(self, jwt_service: JWTAuthService) -> None:
    ...
    def test_verify_token_expired(self, jwt_service: JWTAuthService) -> None:
        """Test token verification with expired token."""
        # Create an already expired token
        data = {"sub": "test-user-123", "exp": int(datetime.utcnow().timestamp()) - 3600}
        pass
        ...
        expired_token = jwt.encode(data, settings.SECRET_KEY, algorithm=settings.JWT_ALGORITHM)

        with pytest.raises(TokenExpiredError, match="Token has expired"):
            jwt_service.verify_token(expired_token)

    def test_verify_token_invalid(self, jwt_service: JWTAuthService) -> None:
...
class TestAuthService:
    ...
    async def test_authenticate_firebase_success(
        self, auth_service: AuthService, authenticated_user: AuthenticatedUser
    ) -> None:
        """Test successful Firebase authentication."""
        request = LoginRequest(token="valid-firebase-token", provider=AuthProvider.FIREBASE)
        pass

        # Mock the Firebase validation
        with patch.object(
            auth_service.firebase_auth, "validate_token", return_value=authenticated_user
        ):
    ...
    @pytest.mark.asyncio
    async def test_authenticate_api_key_success(self, auth_service: AuthService) -> None:
        """Test successful API key authentication."""
        request = LoginRequest(token="valid-api-key-token", provider=AuthProvider.API_KEY)
        pass

        # Mock JWT validation
        with patch.object(
            auth_service.jwt_auth,
            "verify_token",
            return_value={"uid": "api-user-123", "email": "api@example.com", "name": "API User"},
        ):
            result = await auth_service.authenticate(request, "127.0.0.1", "test-agent")
    ...
    @pytest.mark.asyncio
    async def test_authenticate_unsupported_provider(self, auth_service: AuthService) -> None:
        """Test authentication with unsupported provider."""
        request = LoginRequest(token="some-token", provider=AuthProvider.FIREBASE)
        pass
    ...
    async def test_validate_request_token_firebase(
        ...
        request = TokenValidationRequest(token="valid-firebase-token", token_type=TokenType.ACCESS)
        pass
    ...
    @pytest.mark.asyncio
    async def test_validate_request_token_jwt_fallback(self, auth_service: AuthService) -> None:
        """Test token validation falling back to JWT after Firebase fails."""
        request = TokenValidationRequest(token="valid-jwt-token", token_type=TokenType.ACCESS)
        pass

        # Firebase validation fails, JWT validation succeeds
        with (
            patch.object(
                auth_service.firebase_auth,
                "validate_token",
    ...
    @pytest.mark.asyncio
    async def test_validate_request_token_invalid(self, auth_service: AuthService) -> None:
        """Test token validation with completely invalid token."""
        request = TokenValidationRequest(token="invalid-token", token_type=TokenType.ACCESS)
        pass
    ...
    async def test_logout_success(self, auth_service: AuthService) -> None:
        ...
        with patch.object(
            auth_service.firebase_auth, "revoke_tokens", new_callable=AsyncMock, return_value=True
        ):
            result = await auth_service.logout(user_id)
        assert result is True

    @pytest.mark.asyncio
    async def test_logout_failure(self, auth_service: AuthService) -> None:
        """Test logout failure."""
        user_id = "test-user-123"
        pass

        with patch.object(
            auth_service.firebase_auth, "revoke_tokens", new_callable=AsyncMock, return_value=False
        ):
            result = await auth_service.logout(user_id)
        assert result is False

    def test_health_check(self, auth_service: AuthService) -> None:
        """Test auth service health check."""
        with patch.object(auth_service.firebase_auth, "health_check", return_value=True):
