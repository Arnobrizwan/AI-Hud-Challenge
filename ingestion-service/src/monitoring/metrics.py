...
app_info.info({
    'version': '1.0.0',
    'name': 'News Ingestion & Normalization Service'
})
...
request_size = Histogram(
    'ingestion_request_size_bytes',
    'HTTP request size in bytes',
    ['method', 'endpoint']
)

response_size = Histogram(
    'ingestion_response_size_bytes',
...
content_quality_score = Histogram(
    'ingestion_content_quality_score',
...
batch_success_rate = Gauge(
    'ingestion_batch_success_rate',
    'Success rate of processing batches',
    ['source_id']
)
...
pubsub_publish_duration = Histogram(
    'ingestion_pubsub_publish_duration_seconds',
    'Duration of Pub/Sub publish operations',
    ['topic']
)
...
def record_request_metrics(method: str, endpoint: str, status_code: int, duration: float, request_size_bytes: int = 0, response_size_bytes: int = 0):
...
def record_article_processing(source_id: str, source_type: str, status: str, duration: float, word_count: int = 0, reading_time: int = 0, content_type: str = "article"):
