...
class WebScrapingAdapter(BaseAdapter):
    ...
    async def _wait_for_content(self):
        ...
        try:
            ...
            content_selectors = self.scraping_config.get('content_selectors', [
                'article',
                'main',
                '.content',
                '.post',
    ...
    async def _extract_content_from_page(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Title
            title_selectors = self.scraping_config.get('title_selectors', [
                'h1',
                'title',
                '.title',
                '.headline'
            ...
            # Author
            author_selectors = self.scraping_config.get('author_selectors', [
                '.author',
                '.byline',
                '[rel="author"]',
                '.post-author'
            ...
            # Published date
            date_selectors = self.scraping_config.get('date_selectors', [
                'time',
                '.date',
                '.published',
                '.post-date'
            ...
            # Image
            image_selectors = self.scraping_config.get('image_selectors', [
                'img',
                '.featured-image img',
                '.post-image img'
            ])
            ...
            # Tags
            tag_selectors = self.scraping_config.get('tag_selectors', [
                '.tags a',
                '.categories a',
                '.post-tags a'
            ])
            ...
            # Summary
            summary_selectors = self.scraping_config.get('summary_selectors', [
                '.summary',
                '.excerpt',
                '.description'
            ])
