...
from src.models.content import (
    NormalizedArticle, SourceConfig, ProcessingBatch, ProcessingStatus,
    ContentMetrics, DuplicateDetection
)
...
class IngestionService:
    ...
    async def process_source(self, source_config: SourceConfig) -> ProcessingBatch:
        ...
        try:
            ...
            batch.processed_count = len([a for a in processed_articles if a.processing_status == ProcessingStatus.COMPLETED])
            batch.failed_count = len([a for a in processed_articles if a.processing_status == ProcessingStatus.FAILED])
            batch.duplicate_count = len([a for a in processed_articles if a.processing_status == ProcessingStatus.DUPLICATE])
