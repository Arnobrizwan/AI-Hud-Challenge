...
from src.models.content import NormalizedArticle, SourceConfig, ProcessingBatch, ContentMetrics
...
class FirestoreService:
    ...
    async def get_duplicate_articles(self, article: NormalizedArticle, threshold: float = 0.8) -> List[NormalizedArticle]:
        ...
        try:
            ...
            query = self.db.collection(self.articles_collection)
            query = query.where("ingestion_metadata.source_id", "==", article.ingestion_metadata.get('source_id', ''))
            query = query.where("published_at", ">=", date_from)
            query = query.where("published_at", "<=", date_to)
    ...
    async def save_metrics(self, metrics: ContentMetrics) -> bool:
        ...
        try:
            ...
            doc_ref = self.db.collection(self.metadata_collection).document(f"metrics_{metrics.source_id}_{metrics.date}")
