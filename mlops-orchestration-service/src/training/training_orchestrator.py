...
from src.models.training_models import (
    TrainingConfig, TrainingResult, ModelTrainingResult, HyperparameterSpace,
    TrainingData, ModelEvaluationResult, ExperimentConfig
)
...
class ModelTrainingOrchestrator:
    ...
    async def prepare_training_data(self, config: TrainingConfig) -> TrainingData:
        ...
        try:
            ...
            return TrainingData(
                ...
                test_set=test_data,
                feature_count=feature_data.shape[1] - 1,  # Assuming last column is target
    ...
    async def _optimize_hyperparameters(self, config: TrainingConfig, 
        ...
        study = optuna.create_study(
            direction="maximize" if config.optimization_metric in ["accuracy", "f1", "precision", "recall"] else "minimize",
    ...
    async def _train_model(self, config: TrainingConfig, 
                          training_data: TrainingData,
        ...
        try:
            ...
            await self._log_training_metrics(experiment_id, {
    ...
    async def _evaluate_trained_model(self, model: Any, 
                                    training_data: TrainingData,
                                    config: TrainingConfig) -> ModelEvaluationResult:
    ...
    async def _register_model(self, model_result: ModelTrainingResult,
    ...
    async def _custom_training_loop(self, model: Any, training_data: TrainingData,
                                  config: TrainingConfig, callbacks: List[Any]) -> Any:
