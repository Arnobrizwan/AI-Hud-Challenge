...
from src.models.pipeline_models import (
    MLPipeline, MLPipelineConfig, PipelineComponent, PipelineExecution,
    PipelineStatus, PipelineType, ComponentType
)
...
class MLOpsPipelineOrchestrator:
    ...
    async def trigger_pipeline_execution(self, pipeline_id: str,
                                       execution_params: Dict[str, Any]) -> PipelineExecution:
        ...
        try:
            ...
            if pipeline.orchestrator == "airflow":
                ...
                job_id = await self.vertex_ai_client.submit_pipeline_job(
                    pipeline_spec=pipeline.vertex_pipeline_id,
                ...
                run_id = await self.kubeflow_client.submit_pipeline_run(
                    pipeline_id=pipeline.kubeflow_pipeline_id,
                    parameters=execution_params
                )
                execution.external_run_id = run_id
    ...
    async def list_pipelines(self, pipeline_type: Optional[PipelineType] = None,
                           status: Optional[PipelineStatus] = None) -> List[MLPipeline]:
