        ...
        while connection.is_active:
            try:
                ...
                if time_since_last > timedelta(seconds=self.config.timeout):
                    missed_heartbeats += 1
                    logger.warning(f"Missed heartbeat {missed_heartbeats} for connection {connection.id}")
...
class WebSocketManager:
    ...
    async def handle_websocket_message(self, connection: WebSocketConnection, 
    ...
    async def handle_collaboration_event(self, connection: WebSocketConnection, message: Dict[str, Any]):
    ...
    async def handle_user_action(self, connection: WebSocketConnection, message: Dict[str, Any]):
        ...
        if action in ['article_saved', 'preferences_updated']:
            await self.broadcast_to_user(connection.user_id, {
    ...
    async def broadcast_to_group(self, group_id: str, message: Dict[str, Any], 
                               exclude_connection: str = None) -> int:
    ...
    async def send_message(self, connection: WebSocketConnection, 
    ...
    async def send_error(self, connection: WebSocketConnection, 
    ...
    async def get_connections_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connections_by_group": {
                group_id: len(conn_ids) 
                for group_id, conn_ids in self.connection_groups.items()
            }
        }
