...
class HTMLProcessor:
    ...
    async def _extract_with_custom_selectors(self, soup: BeautifulSoup, custom_selectors: Dict[str, str]) -> str:
                    ...
                    additional_content["code_blocks"].append(
                        {
                            "text": code_text,
                            "language": (
                                code.get("class", [""])[0].replace("language-", "") if code.get("class") else ""
                            ),
                            "type": code.name,
                        }
                    )
