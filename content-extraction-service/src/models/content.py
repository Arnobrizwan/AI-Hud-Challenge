...
class LanguageInfo(BaseModel):
    ...
    alternative_languages: List[Dict[str, float]] = Field(default_factory=list, description="Alternative language candidates")
...
class ExtractionStats(BaseModel):
    ...
    @validator("extraction_time_ms", "html_fetch_time_ms", "content_extraction_time_ms", 
               "image_processing_time_ms", "quality_analysis_time_ms", "total_processing_time_ms")
...
class ExtractedContent(BaseModel):
    ...
    def is_high_quality(self, min_quality_score: float = 0.7) -> bool:
        ...
        return (
            self.quality_metrics.overall_quality >= min_quality_score and
            self.quality_metrics.spam_score < 0.3 and
            self.quality_metrics.readability_score >= 30.0 and
            self.word_count >= 100
        )
