...
from ..models.content import (
    ...
    ProcessedImage, VideoMetadata, ExtractionStats, ExtractionMethod,
...
class ContentExtractionPipeline:
    ...
    async def extract_content(
        ...
        try:
            ...
            extraction_stats = self._create_extraction_stats(
                start_time, html_fetch_time, content_extraction_time,
                sanitization_time, image_processing_time, quality_analysis_time,
                language_analysis_time, html_content, processed_images
            )
            
            # Create extracted content object
            extracted_content = self._create_extracted_content(
                request, clean_content, metadata, processed_images,
                quality_metrics, language_info, extraction_stats
            )
    ...
    async def _fetch_web_content(self, request: ExtractionRequest) -> str:
        ...
        try:
            ...
            if request.extraction_method == ExtractionMethod.PLAYWRIGHT:
                ...
                return await playwright_service.fetch_html(
                    request.url,
                ...
                return await http_service.fetch_html(
                    request.url,
                    timeout=request.timeout
                )
        except Exception as e:
    ...
    async def _process_images(self, content: str, request: ExtractionRequest) -> List[ProcessedImage]:
        ...
        try:
            return await self.image_processor.process_images_from_content(
                content, request.url
            )
        except Exception as e:
    ...
    def _create_extracted_content(
        ...
        content_hash = hashlib.sha256(f"{metadata.og_title or ''}|{clean_content}|{request.url}".encode('utf-8')).hexdigest()
