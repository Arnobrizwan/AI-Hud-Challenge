...
class TestDeduplicationPipeline:
    ...
    async def test_process_article_no_duplicates(
            self, deduplication_pipeline, sample_article):
         -> Dict[str, Any]:"""Test processing article with no duplicates."""
    ...
    async def test_process_batch(
            ...
            sample_articles):
         -> Dict[str, Any]:"""Test batch processing."""
        ...
        assert all(result.article_id == article.id for result,
                   article in zip(results, sample_articles))

    @pytest.mark.asyncio
    async def test_find_lsh_candidates(
            self, deduplication_pipeline, sample_article):
         -> Dict[str, Any]:"""Test LSH candidate finding."""
        # Mock the LSH index query
        deduplication_pipeline.lsh_index.query_candidates = AsyncMock(
            return_value=[])
    ...
    async def test_filter_semantic_matches(
        self, deduplication_pipeline, sample_article, sample_articles
         ...
         -> Dict[str, Any]:"""Test semantic similarity filtering."""
    ...
    async def test_classify_duplicates(
            self, deduplication_pipeline, sample_article):
         -> Dict[str, Any]:"""Test duplicate classification."""
        ...
        deduplication_pipeline._get_article_by_id = AsyncMock(
    ...
    async def test_update_indices(
            ...
            deduplication_pipeline,
            sample_article):
         -> Dict[str, Any]:"""Test index updates."""
        content_fingerprint = {
...
class TestDeduplicationService:
    ...
    async def test_deduplicate_article(
            self, deduplication_service, sample_article):
         -> Dict[str, Any]:"""Test single article deduplication."""
    ...
    async def test_deduplicate_batch(
            ...
            sample_articles):
         -> Dict[str, Any]:"""Test batch deduplication."""
    ...
    async def test_find_duplicates(
            ...
            deduplication_service,
            sample_article):
         -> Dict[str, Any]:"""Test finding duplicates."""
        # Mock the pipeline's _find_lsh_candidates method
        deduplication_service.pipeline._find_lsh_candidates = AsyncMock(
            return_value=[])
        ...
        assert all(isinstance(sim, SimilarityScore)
