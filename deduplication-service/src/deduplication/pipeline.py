...
from ..algorithms.lsh.minhash import MinHashGenerator, ContentFingerprinter
...
from ..models.schemas import NormalizedArticle, DuplicateResult, SimilarityScore
...
class DeduplicationPipeline:
    ...
    async def _filter_semantic_matches(
        ...
        similarities = await self.similarity_calc.compute_batch_similarity(
            article, candidates
        )
    ...
    async def _classify_duplicates(
        self,
        article: NormalizedArticle,
        semantic_matches: List[SimilarityScore]
    ) -> List[SimilarityScore]:
    ...
    async def _get_or_create_cluster(
        self,
        article: NormalizedArticle,
        duplicates: List[SimilarityScore]
    ) -> Optional[UUID]:
