...
class EventGroupingEngine:
    ...
    async def group_into_events(
        ...
        for cluster_id in set(cluster_labels):
            ...
            cluster_articles = [
                article for article, label in zip(articles, cluster_labels)
                if label == cluster_id
            ]
    ...
    async def _compute_representative_score(
        ...
        cluster_articles: List[NormalizedArticle]
    ) -> float:
    ...
    async def _compute_centrality(
        ...
        for other_article in cluster_articles:
            if other_article.id != candidate.id:
                similarity = await self.semantic_calc.compute_similarity(
                    candidate, other_article
                )
