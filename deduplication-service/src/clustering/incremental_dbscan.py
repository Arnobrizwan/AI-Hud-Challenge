...
class IncrementalDBSCAN:
    ...
    async def _compute_topic_features(self, topics: List[Topic]) -> np.ndarray:
        ...
        return np.array([topic_count, avg_confidence, unique_topics, high_conf_topics, entropy])
    ...
    async def _compute_geo_features(self, locations: List[Location]) -> np.ndarray:
        ...
        return np.array([location_count, country_count, region_count, avg_confidence, geo_spread])

    async def _update_clustering_state(
        self, articles: List[NormalizedArticle], cluster_labels: np.ndarray, features: np.ndarray
    ) -> None:
        ...
        for cluster_id, article_ids in self.clusters.items():
            cluster_indices = [i for i, article in enumerate(articles) if article.id in article_ids]
    ...
    async def _update_cluster_center(self, cluster_id: int, new_features: np.ndarray) -> None:
