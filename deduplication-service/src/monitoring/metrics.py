...
class MetricsCollector:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.cluster_size_histogram = Histogram(
            "cluster_size", "Distribution of cluster sizes", registry=self.registry
        )
        ...
        self.active_clusters_gauge = Gauge(
            "active_clusters", "Number of active clusters", registry=self.registry
        )
    ...
    async def record_histogram(
        ...
        await self.redis.ltrim(f"metrics:histograms:{key}", 0, 999)  # Keep last 1000 values
