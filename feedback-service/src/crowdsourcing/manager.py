...
from ..models.schemas import Campaign, CampaignCreate, CampaignTask, CampaignSubmission
from ..models.database import Campaign as CampaignDB, CampaignTask as CampaignTaskDB
...
class CrowdsourcingManager:
    ...
    async def create_campaign(self, campaign: CampaignCreate, 
    ...
    async def create_campaign_tasks(self, campaign_id: UUID, 
                                  content_ids: List[UUID]) -> List[CampaignTask]:
        ...
        try:
            ...
            logger.info("Campaign tasks created", 
    ...
    async def submit_campaign_response(self, task_id: UUID, 
                                     worker_id: str,
        ...
        try:
            ...
            logger.info("Campaign response submitted", 
                       task_id=str(task_id), 
                       worker_id=worker_id,
    ...
    async def process_campaign_results(self, campaign_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get all submissions
            result = await self.db.execute(
                select(CampaignSubmission).join(CampaignTaskDB).where(
                    CampaignTaskDB.campaign_id == campaign_id
                )
            )
            ...
            metrics = {
                ...
                "average_quality_score": sum(s.quality_score for s in validated_submissions) / len(validated_submissions) if validated_submissions else 0,
                "completion_rate": len(validated_submissions) / len(submissions) if submissions else 0
            }
            
            logger.info("Campaign results processed", 
                       campaign_id=str(campaign_id), 
                       metrics=metrics)
