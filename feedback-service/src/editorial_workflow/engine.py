...
from ..models.schemas import (
    ReviewTask, ReviewTaskCreate, ReviewResult, ReviewResultCreate,
    TaskStatus, TaskPriority, ReviewDecision
)
from ..models.database import ReviewTask as ReviewTaskDB, ReviewResult as ReviewResultDB, User
...
class EditorialWorkflowEngine:
    ...
    async def create_review_task(self, content_id: UUID, task_type: str, 
                               priority: str = "normal", created_by: Optional[UUID] = None) -> ReviewTask:
    ...
    async def process_review_completion(self, task_id: UUID, 
                                      review_result: ReviewResultCreate) -> Dict[str, Any]:
    ...
    async def get_task(self, task_id: UUID) -> Optional[ReviewTaskDB]:
        ...
        result = await self.db.execute(
            select(ReviewTaskDB).where(ReviewTaskDB.id == task_id)
        )
    ...
    async def trigger_post_review_workflows(self, task: ReviewTaskDB, 
                                          review_result: ReviewResultCreate) -> None:
        ...
        logger.info("Post-review workflows triggered", 
                   task_id=str(task.id), 
                   decision=review_result.decision.value)
    
    async def notify_review_completion(self, task: ReviewTaskDB, 
                                     review_result: ReviewResultCreate) -> None:
...
class ApprovalEngine:
    ...
    async def process_approval_chain(self, content_id: UUID, 
