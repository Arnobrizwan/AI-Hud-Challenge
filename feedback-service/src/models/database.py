...
from sqlalchemy import Column, String, Text, Integer, Float, Boolean, DateTime, ForeignKey, JSON, Enum as SQLEnum, Index
...
from .schemas import (
    FeedbackType, SignalType, TaskStatus, TaskPriority, 
    ReviewDecision, AnnotationType, CampaignStatus, QualityLevel
)
...
class User(Base):
    ...
    review_tasks_assigned = relationship("ReviewTask", foreign_keys="ReviewTask.assigned_to", back_populates="assigned_user")
    review_tasks_created = relationship("ReviewTask", foreign_keys="ReviewTask.created_by", back_populates="created_by_user")
...
class Annotation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    task_id = Column(UUID(as_uuid=True), ForeignKey("annotation_tasks.id", ondelete="CASCADE"), nullable=False, index=True)
...
class CampaignSubmission(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    task_id = Column(UUID(as_uuid=True), ForeignKey("campaign_tasks.id", ondelete="CASCADE"), nullable=False, index=True)
...
Index('idx_performance_metrics_model_timestamp', PerformanceMetric.model_name, PerformanceMetric.timestamp)
