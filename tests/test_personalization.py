...
from src.personalization.engine import (
    PersonalizationEngine, UserProfileManager, 
    CollaborativeFilter, ContentBasedFilter
)
from src.schemas import Article, UserProfile, Source, Author, Topic
...
def sample_article():
    ...
    return Article(
        ...
        topics=[
            Topic(name="technology", confidence=0.8),
            Topic(name="AI", confidence=0.9)
        ],
        word_count=500,
        reading_time=2,
        quality_score=0.8
...
async def test_compute_source_preference(personalization_engine):
    ...
    score = await personalization_engine._compute_source_preference(
        source_id, source_preferences
    )
