...
from src.schemas import RankingRequest, Article, UserProfile
...
def test_get_experiments_endpoint(client):
    ...
    with patch('src.main.ab_framework') as mock_framework:
        mock_framework.get_all_experiments = AsyncMock(return_value=[
            {
...
def test_get_experiment_stats_endpoint(client):
    ...
    with patch('src.main.ab_framework') as mock_framework:
        mock_framework.get_experiment_stats = AsyncMock(return_value={
            "experiment_id": "test_experiment",
            "total_users": 100,
            "variants": {}
        })
