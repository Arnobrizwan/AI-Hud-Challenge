...
from src.models.schemas import NotificationType, FatigueCheck
...
async def test_fatigue_score_calculation(fatigue_detector):
    ...
    for hourly, daily, user_fatigue, expected_min in test_cases:
        # Mock Redis
        fatigue_detector.redis_client.get = AsyncMock(side_effect=[str(hourly), str(daily), str(user_fatigue)])
...
async def test_different_notification_types_fatigue_limits(fatigue_detector):
    ...
    result_personalized = await fatigue_detector.check_fatigue("user123", NotificationType.PERSONALIZED)
