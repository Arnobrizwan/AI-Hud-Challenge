...
from ..models.schemas import (
    ...
    NotificationType, DeliveryChannel, Priority
)
...
class NotificationDecisionEngine:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                strategy_variant = await self.ab_tester.get_variant(
                    user_id=candidate.user_id,
                    experiment="notification_strategy_v3"
                )
    ...
    async def process_batch_notifications(
        ...
        tasks = [
            self.process_notification_candidate(candidate)
        ...
        for i, decision in enumerate(decisions):
            if isinstance(decision, Exception):
                logger.error(
                    "Error processing candidate in batch",
                    candidate_index=i,
                    error=str(decision)
                )
                processed_decisions.append(NotificationDecision(
                    should_send=False,
