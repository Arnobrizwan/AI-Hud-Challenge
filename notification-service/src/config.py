...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = Field(
        default="postgresql://user:password@localhost/notification_db",
        env="DATABASE_URL"
    )
    ...
    # ML Model settings
    TIMING_MODEL_PATH: str = Field(
        default="models/timing_model.pkl",
        env="TIMING_MODEL_PATH"
    )
    RELEVANCE_MODEL_PATH: str = Field(
        default="models/relevance_model.pkl",
        env="RELEVANCE_MODEL_PATH"
    )
    ENGAGEMENT_MODEL_PATH: str = Field(
        default="models/engagement_model.pkl",
        env="ENGAGEMENT_MODEL_PATH"
    )
    ...
    AB_TEST_EXPERIMENTS: List[str] = Field(
        default=["notification_strategy_v3", "timing_optimization_v2"],
        env="AB_TEST_EXPERIMENTS"
    )
