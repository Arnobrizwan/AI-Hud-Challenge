...
from ..models.schemas import (
    NotificationCandidate, NotificationDecision, DeliveryResult,
    BatchNotificationRequest, BatchNotificationResponse,
    NotificationPreferences, UserProfile, ABTestVariant,
    NewsItem, NotificationType, DeliveryChannel
)
...
from ..exceptions import NotificationError, NotificationDeliveryError
...
async def deliver_notification(
    ...
    try:
        if not decision.should_send:
            raise HTTPException(
                status_code=400, 
                detail="Cannot deliver notification that should not be sent"
            )
...
async def process_breaking_news(
    ...
    try:
        ...
        background_tasks.add_task(
            _process_breaking_news_candidates,
            candidates,
            decision_engine
        )
        
        logger.info(
            "Breaking news processed",
            news_item_id=news_item.id,
...
async def _process_breaking_news_candidates(
    ...
    try:
        logger.info(
            "Processing breaking news candidates in background",
        ...
        logger.error(
            "Error processing breaking news candidates in background",
            error=str(e),
            exc_info=True
        )
...
async def get_user_fatigue_analytics(
    ...
    try:
        ...
        logger.error(
            "Error getting fatigue analytics",
            user_id=user_id,
            error=str(e)
        )
...
async def get_delivery_analytics(
    ...
    try:
        ...
        logger.error(
            "Error getting delivery analytics",
            user_id=user_id,
            error=str(e)
        )
...
async def get_user_experiments(
    ...
    try:
        ...
        logger.error(
            "Error getting user experiments",
            user_id=user_id,
            error=str(e)
        )
