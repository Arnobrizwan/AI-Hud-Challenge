...
# Check if we're running fast or full profile
if docker-compose -f docker-compose.dev.yml ps | grep -q "foundations-guards"; then
  # Fast profile - core services only
  services=(
