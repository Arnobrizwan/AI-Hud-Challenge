...
def update_grafana_dashboard():
    ...
    auth_data = {"name": "api-key", "role": "Admin"}

    try:
        # Get API key
        response = requests.post(auth_url, json=auth_data, auth=(grafana_user, grafana_password))
        ...
        response = requests.post(dashboard_url, json=dashboard_config, headers=headers)
