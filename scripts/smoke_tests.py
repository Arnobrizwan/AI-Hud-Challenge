...
def main():
    ...
    parser.add_argument(
        "--environment", default="production", choices=["production", "staging", "local"], help="Environment to test"
    )
    parser.add_argument("--base-url", help="Override base URL for testing")
    parser.add_argument("--timeout", type=int, default=30, help="Timeout for each service check")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        health_futures = {
            executor.submit(test_service_health, service, f"{base_url}/{service}"): service for service in SERVICES
        }
        ...
        endpoint_futures = {
            executor.submit(test_service_endpoints, service, f"{base_url}/{service}"): service for service in SERVICES
        }
